...
final class translentor_currency_FIXED_USER_ROLE extends translentor_currency_FIXED_AMOUNT {
    ...
    public function woocommerce_product_options_general_product_data() {
        ...
        if ($_product->is_type('simple') OR $_product->is_type('external') OR $_product->is_type('subscription')) {
            ...
            $data['is_fixed_enabled'] = $translentor_currency->is_fixed_user_role;
            $data['post_id'] = $post->ID;
            $data['type'] = 'simple';
            $data['user_role_data'] = $this->get_product_user_role_data($post->ID);

            echo $this->render_html(translentor_currency_PATH . 'views/fixed/product_user_role_data.php', $data);
        }
    }
    ...
    public function save_product_prices($post_id) {
        ...
        if (isset($_POST['translentor_currency_price_user_role_name'])) {
            update_post_meta($post_id, '_translentor_currency_price_user_role_name', '');
            update_post_meta($post_id, '_translentor_currency_regular_price_user_role', '');
            update_post_meta($post_id, '_translentor_currency_sale_price_user_role', '');
            if (is_array($_POST['translentor_currency_price_user_role_name'])) {
                foreach ($_POST['translentor_currency_price_user_role_name'] as $post_id => $rules) {
                    update_post_meta($post_id, '_translentor_currency_price_user_role_name', $rules);
                }

                foreach ($_POST['translentor_currency_regular_price_user_role'] as $post_id => $rules) {
                    if (is_array($rules)) {
                        foreach ($rules as &$val) {
                            $val = $this->prepare_float_val($val);
                        }
                        update_post_meta($post_id, '_translentor_currency_regular_price_user_role', $rules);
                    }
                }

                foreach ($_POST['translentor_currency_sale_price_user_role'] as $post_id => $rules) {
                    if (is_array($rules)) {
                        foreach ($rules as &$val) {
                            $val = $this->prepare_float_val($val);
                        }
                        update_post_meta($post_id, '_translentor_currency_sale_price_user_role', $rules);
                    }
                }
            }
        }
    ...
    public function get_product_user_role_data($post_id) {
        $data = array();
        $data['price_user_role_name'] = (array) get_post_meta($post_id, '_translentor_currency_price_user_role_name', true);
        $data['regular_price_user_role'] = (array) get_post_meta($post_id, '_translentor_currency_regular_price_user_role', true);
        $data['sale_price_user_role'] = (array) get_post_meta($post_id, '_translentor_currency_sale_price_user_role', true);
    ...
    public function get_value($post_id, $code, $type) {
        ...
        if (is_user_logged_in()) {
            ...
            $roles = (array) get_post_meta($post_id, '_translentor_currency_price_user_role_name', true);
            if (!empty($roles)) {
                $curr_key = "";
                ...
                if ($curr_key) {
                    $prices = (array) get_post_meta($post_id, '_translentor_currency_' . $type . '_price_user_role', true);
        ...
        return apply_filters('translentor_currency_fixed_price_user_rope', $value, $post_id, $type, $user_roles);
    }
