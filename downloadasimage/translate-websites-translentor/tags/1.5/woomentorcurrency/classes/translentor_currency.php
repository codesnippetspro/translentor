...
final class translentor_currency {
    public $storage = null;
    public $cron = NULL;
    public $cron_hook = 'translentor_currency_update_rates_wpcron';
    ...
    public function __construct() {
        ...
        //update_option ('translentor_currency_collect_statistic', 0);
        define('translentor_currency_MEMCACHED_SERVER', get_option('wmc_storage_server', 'localhost'));
        define('translentor_currency_MEMCACHED_PORT', intval(get_option('wmc_storage_port', 11211)));
        define('translentor_currency_REDIS_SERVER', get_option('wmc_storage_server', 'localhost'));
        define('translentor_currency_REDIS_PORT', intval(get_option('wmc_storage_port', 6379)));

        $this->storage = new wmc_storage(get_option('wmc_storage', 'cookie'));
        $this->statistic = new translentor_currency_STATISTIC();
        //profiles
        $this->geoip_profiles = new translentor_currency_Profile('translentor_currency_geoip_profiles_data');
        add_action('wp_ajax_translentor_currency_update_profiles_data', array($this, 'update_profiles_data'));
        add_action('wp_ajax_translentor_currency_delete_profiles_data', array($this, 'delete_profiles_data'));

        $this->init_no_cents();
        if (!defined('DOING_AJAX')) {
//we need it if shop uses cache plugin, in such way prices will be redraw by AJAX
            $this->shop_is_cached = get_option('translentor_currency_shop_is_cached', 0);
            $this->shop_is_cached_preloader = get_option('translentor_currency_shop_is_cached_preloader', 0);
        }

 
        $this->actualized_for = floatval(get_option('translentor_currency_currency_version', 3.2));
...
//+++
        ...
        $this->is_geoip_manipulation = get_option('translentor_currency_is_geoip_manipulation', 0);
        $this->is_multiple_allowed = get_option('translentor_currency_is_multiple_allowed', 0);
        if ($this->is_geoip_manipulation) {
            $this->is_multiple_allowed = true;
        }
        $this->is_fixed_enabled = get_option('translentor_currency_is_fixed_enabled', 0);
        $this->is_fixed_coupon = get_option('translentor_currency_is_fixed_coupon', 0);
        $this->is_fixed_shipping = get_option('translentor_currency_is_fixed_shipping', 0);
        $this->is_fixed_shipping_free = get_option('translentor_currency_is_fixed_shipping', 0);
        $this->is_fixed_user_role = get_option('translentor_currency_is_fixed_user_role', 0);

        $this->force_pay_bygeoip_rules = get_option('translentor_currency_force_pay_bygeoip_rules', 0);
        $this->rate_auto_update = get_option('translentor_currency_currencies_rate_auto_update', 'no');
...
//+++
        if (!intval(get_option('translentor_currency_first_activation', 0))) {
            update_option('translentor_currency_first_activation', 1);
            update_option('translentor_currency_drop_down_view', 'ddslick');
            update_option('translentor_currency_currencies_aggregator', 'yahoo');
            update_option('translentor_currency_aggregator_key', '');
            update_option('translentor_currency_welcome_currency', $this->default_currency);
            update_option('translentor_currency_is_multiple_allowed', 1);
            update_option('translentor_currency_is_fixed_enabled', 0);
            update_option('translentor_currency_is_fixed_shipping', 0);
            update_option('translentor_currency_is_fixed_coupon', 0);
            update_option('translentor_currency_is_fixed_user_role', 0);
            update_option('translentor_currency_force_pay_bygeoip_rules', 0);
            update_option('translentor_currency_is_geoip_manipulation', 0);
            update_option('translentor_currency_collect_statistic', 0);
            update_option('translentor_currency_show_top_button', 0);
            update_option('translentor_currency_activate_page_list', "");
            update_option('translentor_currency_activate_page_list_reverse', 0);
            update_option('translentor_currency_show_flags', 1);
            update_option('translentor_currency_special_ajax_mode', 0);
            update_option('translentor_currency_show_money_signs', 1);
            update_option('translentor_currency_customer_signs', '');
            update_option('translentor_currency_customer_price_format', '');
            update_option('translentor_currency_currencies_rate_auto_update', 'no');
            update_option('translentor_currency_rate_auto_update_email', 0);
            update_option('wmc_storage', 'cookie');
            update_option('translentor_currency_geo_rules', '');

            update_option('translentor_currency_payments_rule_enabled', '0');
            update_option('translentor_currency_payment_control', '0');
            update_option('translentor_currency_payments_rules', '');

            update_option('translentor_currency_hide_cents', '');
            update_option('translentor_currency_hide_on_front', '');
            update_option('translentor_currency_rate_plus', '');
            update_option('translentor_currency_decimals', []);
            update_option('translentor_currency_separators', []);
            update_option('translentor_currency_price_info', 0);
            update_option('translentor_currency_no_cents', '');
            update_option('translentor_currency_restrike_on_checkout_page', 0);
            update_option('translentor_currency_shop_is_cached', 0);
            update_option('translentor_currency_shop_is_cached_preloader', 0);
            update_option('translentor_currency_show_approximate_amount', 0);
            update_option('translentor_currency_show_approximate_price', 0);

            //auto swither
            update_option('translentor_currency_is_auto_switcher', 0);
            update_option('translentor_currency_auto_switcher_show_page', '');
            update_option('translentor_currency_auto_switcher_hide_page', '');
            update_option('translentor_currency_auto_switcher_mobile_show', 0);

            update_option('wmc_storage_server', 'localhost');
            update_option('wmc_storage_port', 11211);
            update_option('translentor_currency_admin_theme_id', 1); //all new customers let use new admin panel


            $geoIP_profiles = array('translentor_currency_profile_def_p777' => array(
                    'name' => esc_html__('EU countries', 'woomentor-currency'),
...
//simple checkout itercept
        if (isset($_REQUEST['action']) AND $_REQUEST['action'] == 'woocommerce_checkout') {
            $_REQUEST['woomentor-currency'] = sanitize_text_field($this->storage->get_val('translentor_currency_current_currency'));
            $this->current_currency = sanitize_text_field($this->storage->get_val('translentor_currency_current_currency'));
            $_REQUEST['translentor_currency_in_order_currency'] = $this->current_currency;
        }
...
//paypal query itercept
        if (isset($_REQUEST['mc_currency']) AND!empty($_REQUEST['mc_currency'])) {
            if (array_key_exists($_REQUEST['mc_currency'], $currencies)) {
                $_REQUEST['woomentor-currency'] = sanitize_text_field($_REQUEST['mc_currency']);
            }
        }

//WELCOME USER CURRENCY ACTIVATION
        if (intval($this->storage->get_val('translentor_currency_first_unique_visit')) === 0) {
            $this->is_first_unique_visit = true;
            $this->set_currency($this->get_welcome_currency());
            setcookie('translentor_currency_first_unique_visit', 1, time() + 1 * 24 * 3600);
            //$this->storage->set_val('translentor_currency_first_unique_visit', 1);
        }
        ...
        if (isset($_GET['currency']) AND!empty($_GET['currency'])) {
            ...
            if (!$this->is_currency_private($_GET['currency'])) {
                ...
                //1 issue closing
                if (!get_option('translentor_currency_is_multiple_allowed', 0)) {
                ...
                if (array_key_exists(strtoupper($_GET['currency']), $currencies) AND $allow_currency_switching) {
                    //$this->storage->set_val('translentor_currency_current_currency', strtoupper(sanitize_text_field($_GET['currency'])));
                    $this->statistic->register_switch(strtoupper(sanitize_text_field($_GET['currency'])), strtoupper($this->storage->get_val('translentor_currency_user_country')));
                }
            }
        }
//+++
        if ($this->storage->is_isset('translentor_currency_current_currency')) {
            $this->current_currency = $this->storage->get_val('translentor_currency_current_currency');
        } else {
            $this->current_currency = $this->default_currency;
        }
        //$this->storage->set_val('translentor_currency_default_currency', $this->default_currency);
        setcookie('translentor_currency_default_currency',$this->default_currency, time() + 1 * 24 * 3600);
//+++
        //if we want to be paid in the basic currency - not multiple mode
        if (isset($_REQUEST['action']) AND!get_option('translentor_currency_is_multiple_allowed', 0)) {
...
//+++ FILTERS
        ...
        add_filter('wc_get_template', array($this, 'wc_get_template'), 9999, 5); //from data 2.7 its nessesary for new order email
...
//+++ AJAX ACTIONS
        ...
        add_action('woocommerce_before_calculate_totals', array($this, 'translentor_currency_before_calculate_totals_geoip_fix'));

        add_action('wp_ajax_translentor_currency_save_etalon', array($this, 'save_etalon'));
        

        add_action('wp_ajax_translentor_currency_convert_currency', array($this, 'translentor_currency_convert_currency'));
        add_action('wp_ajax_nopriv_translentor_currency_convert_currency', array($this, 'translentor_currency_convert_currency'));

        add_action('wp_ajax_translentor_currency_rates_current_currency', array($this, 'translentor_currency_rates_current_currency'));
        add_action('wp_ajax_nopriv_translentor_currency_rates_current_currency', array($this, 'translentor_currency_rates_current_currency'));

        add_action('wp_ajax_translentor_currency_get_products_price_html', array($this, 'translentor_currency_get_products_price_html'));
        add_action('wp_ajax_nopriv_translentor_currency_get_products_price_html', array($this, 'translentor_currency_get_products_price_html'));

        add_action('wp_ajax_translentor_currency_get_variation_products_price_html', array($this, 'translentor_currency_get_variation_products_price_html'));
        add_action('wp_ajax_nopriv_translentor_currency_get_variation_products_price_html', array($this, 'translentor_currency_get_variation_products_price_html'));

        add_action('wp_ajax_translentor_currency_get_custom_price_html', array($this, 'translentor_currency_get_custom_price_html'));
        add_action('wp_ajax_nopriv_translentor_currency_get_custom_price_html', array($this, 'translentor_currency_get_custom_price_html'));

        add_action('wp_ajax_translentor_currency_recalculate_order_data', array($this, 'translentor_currency_recalculate_order_data'));
        add_action('wp_ajax_translentor_currency_all_order_ids', array($this, 'translentor_currency_all_order_ids'));
        add_action('wp_ajax_translentor_currency_recalculate_orders_data', array($this, 'translentor_currency_recalculate_orders_data'));

        add_action('wp_ajax_translentor_currency_set_currency_ajax', array($this, 'translentor_currency_set_currency_ajax'));
        add_action('wp_ajax_nopriv_translentor_currency_set_currency_ajax', array($this, 'translentor_currency_set_currency_ajax'));

        add_action('wp_ajax_translentor_currency_admin_theme_id', function () {
            update_option('translentor_currency_admin_theme_id', intval($_REQUEST['theme_id']));
        });
...
//*** additional
        ...
        add_action('translentor_currency_exchange_value', array($this, 'translentor_currency_exchange_value'), 1);
        //***
        ...
        add_filter('wc_get_price_decimals', array($this, 'translentor_currency_fix_decimals'), 999);

//*************************************
        if (!isset($_REQUEST['legacy-widget-preview'])) {//tmp fix for wp 5.8
            add_shortcode('translentor_currency', array($this, 'translentor_currency_shortcode'));
            add_shortcode('translentor_currency_get_sign_rate', array($this, 'get_sign_rate'));
            add_shortcode('translentor_currency_converter', array($this, 'translentor_currency_converter'));
            add_shortcode('translentor_currency_rates', array($this, 'translentor_currency_rates'));
            add_shortcode('translentor_currency_show_current_currency', array($this, 'translentor_currency_show_current_currency'));
            add_shortcode('translentor_currency_show_custom_price', array($this, 'translentor_currency_show_custom_price'));
            add_shortcode('translentor_currency_geo_hello', array($this, 'translentor_currency_geo_hello'));
            add_shortcode('translentor_currency_price', array($this, 'translentor_currency_price_shortcode'));
        }
...
//***
        if ($this->is_fixed_enabled OR $this->is_geoip_manipulation) {
            $this->fixed = new translentor_currency_FIXED_PRICE();
        }
        if ($this->is_fixed_coupon) {
            $this->fixed_coupon = new translentor_currency_FIXED_COUPON();
        }
        if ($this->is_fixed_shipping) {
            $this->fixed_shipping = new translentor_currency_FIXED_SHIPPING();
            $this->fixed_shipping_free = new translentor_currency_FIXED_SHIPPING_FREE();
        }
        if ($this->is_fixed_user_role) {
            $this->fixed_user_role = new translentor_currency_FIXED_USER_ROLE();
        }
       

        //adapt_filter
        add_filter('translentor_currency_convert_price', array($this, 'translentor_currency_convert_price'), 10, 2);
        add_filter('translentor_currency_back_convert_price', array($this, 'translentor_currency_back_convert_price'), 10, 2);
        add_filter('translentor_currency_convert_price_wcdp', array($this, 'translentor_currency_convert_price_wcdp'), 10, 3);

        //payments rule
        if (get_option('translentor_currency_payments_rule_enabled', 0)) {
            add_filter('woocommerce_available_payment_gateways', array($this, 'translentor_currency_filter_gateways'), 10, 1);
        }
        // marketing alert
        //add_action('init', array($this, 'init_marketig_translentor_currency'));
        ...
        //REST API
        //wp-json/translentor_currency/v3/currency
        add_action('rest_api_init', function () {
            register_rest_route('translentor_currency/v3', '/currency', array(
                'methods' => 'GET',
                'callback' => function () {
                    global $translentor_currency;
                    return $translentor_currency->get_currencies();
                },
        ...
        if (function_exists('is_admin') AND is_admin()) {
            new translentor_currency_reports();
        }
        $act_stat = new translentor_currency_currency_stat();
        $act_stat->init();
        //***
    }
...
//+++
        ...
        try {
            ...
            if (function_exists('determine_locale')) {
                $locale = apply_filters('plugin_locale', determine_locale(), 'woomentor-currency');
                unload_textdomain('woomentor-currency');

                if (is_file("{$lang_dir}woomentor-currency-{$locale}.mo")) {
                    load_textdomain('woomentor-currency', "{$lang_dir}woomentor-currency-{$locale}.mo");
                } else {
                    if (is_file(translentor_currency_PATH . "languages/woomentor-currency-{$locale}.mo")) {
                        load_textdomain('woomentor-currency', translentor_currency_PATH . "languages/woomentor-currency-{$locale}.mo");
                    } else {
                        load_plugin_textdomain('woomentor-currency', false, translentor_currency_PATH . 'languages');
                    }
                }
            }
        } catch (Exception $e) {
...
//check if is order has installed currency
                $currency = get_post_meta(sanitize_text_field($_REQUEST['order_id']), '_order_currency', TRUE);
                if (!empty($currency)) {
        ...
        if (wp_doing_ajax() AND isset($_REQUEST['action'])
            ...
            $currency = get_post_meta(sanitize_text_field($_REQUEST['order_id']), '_order_currency', true);
            if (!empty($currency)) {
                $_REQUEST['translentor_currency_in_order_currency'] = $currency;
                $this->current_currency = $currency;
            }
        }
...
//compatibility for WC_Gateway_PayPal_Express_AngellEYE
        ...
        if ($this->is_use_geo_rules() AND get_option('translentor_currency_show_approximate_amount', 0) AND ( isset(WC()->cart)/* AND WC()->cart->subtotal > 0 */)) {
        ...
        if (apply_filters('translentor_currency_cut_cart_price_format', true)) {
            add_action('woocommerce_cart_item_price', array($this, 'translentor_currency_woocommerce_cart_price_html'), 999, 2);
            add_filter('woocommerce_cart_item_subtotal', array($this, 'translentor_currency_woocommerce_cart_price_html'), 999, 2);
            add_filter('woocommerce_cart_subtotal', array($this, 'translentor_currency_woocommerce_cart_price_html'), 999, 2);
            add_filter('woocommerce_cart_total', array($this, 'translentor_currency_woocommerce_cart_price_html'), 999, 2);
        }
            ...
            foreach ($data as $k => $d) {
                if (isset($d['id'])) {
                    ...
                    if ($d['id'] === 'pricing_options') {
                        $data[$k]['desc'] = sprintf(__('The following options affect how prices are displayed on the frontend. WooCommerce Currency Switcher is activated. Set default currency %s please.', 'woomentor-currency'), '<a href="' . admin_url('admin.php?page=wc-settings&tab=translentor_currency') . '">' . esc_html__('here', 'woomentor-currency') . '</a>');
                    }
                }
            }
    ...
    public function make_rates_auto_update($reset = false) {
        if ($this->rate_auto_update != 'no' AND!empty($this->rate_auto_update)) {
            if ($this->wp_cron_period) {
                ...
                $this->translentor_currency_wpcron_init();
            }
        }
    }

    public function translentor_currency_wpcron_init($remove = false) {
        if ($remove) {
    ...
    public function get_translentor_currency_cron_schedules($key = '') {
        $schedules = array(
    ...
    public function init_currency_symbols() {
        ...
        $this->currency_symbols = apply_filters('translentor_currency_currency_symbols', array_merge($this->currency_symbols, $this->get_customer_signs()));
    }

    private function init_no_cents() {
        $no_cents = get_option('translentor_currency_no_cents', '');
        $currencies = $this->get_currencies();
//***
        if (!empty($currencies) AND is_array($currencies)) {
...
//*** checking and assigning data
        ...
        if (isset($_REQUEST['translentor_currency_cron_running']) AND!$email_is_sent) {
            if (get_option('translentor_currency_rate_auto_update_email', 0)) {
                $message = sprintf('<strong>' . esc_html__('Base currency of the site is: %s', 'woomentor-currency') . '</strong>', $this->default_currency);
                $message .= '<br /><br /><ul>';
        ...
        update_option('translentor_currency', $currencies);
    }
    ...
    public function init_geo_currency() {
        ...
        if ($this->is_use_geo_rules()) {
            ...
            if (apply_filters("translentor_currency_geobone_ip", true)) {
                if (intval($this->storage->get_val('translentor_currency_first_unique_geoip')) === 0) {
                    $this->is_first_unique_visit = true;
                    //setcookie('translentor_currency_first_unique_geoip', 1, time() + 1 * 24 * 3600);
                }
            }
    ...
    public function register_modules()
    {
        add_action( 'elementor/widgets/widgets_registered', function() 
        {
            ...
            require translentor_currency_PATH . 'classes/widgets/currency.php';
        });
    }
    ...
    public function admin_head() {
        if (isset($_GET['translentor_currency_reset'])) {
            delete_option('translentor_currency');
        }

        //orders
        global $typenow;
        if (function_exists("wc_get_order_types") AND in_array($typenow, wc_get_order_types('order-meta-boxes'), true)) {
            wp_enqueue_script('translentor_currency-orders-script', translentor_currency_LINK . 'js/orders.min.js', array('jquery'), translentor_currency_VERSION);
        }
    ...
    public function admin_init() {
        if (get_option('translentor_currency_is_multiple_allowed', 0)) {
            add_meta_box('translentor_currency_order_metabox', esc_html__('translentor_currency Order Info', 'woomentor-currency'), array($this, 'translentor_currency_order_metabox'), 'shop_order', 'side', 'default');

            add_meta_box('translentor_currency_order_metabox_wcs', esc_html__('translentor_currency Order Info', 'woomentor-currency'), array($this, 'translentor_currency_order_metabox'), 'shop_subscription', 'side', 'default');
        }
    }
    ...
    public function save_post($order_id) {
        if (current_user_can('edit_shop_orders')) {
            if (!empty($_POST)) {
                global $post;
                if (is_object($post)) {
                    if (($post->post_type == 'shop_order' || $post->post_type == 'shop_subscription') AND isset($_POST['translentor_currency_order_currency'])) {
                        $currencies = $this->get_currencies();
                        $currencies_keys = array_keys($currencies);
                        $currency = sanitize_text_field($_POST['translentor_currency_order_currency']);
                        if (in_array($currency, $currencies_keys)) {

//changing order currency
                            update_post_meta($order_id, '_order_currency', $currency);

                            update_post_meta($order_id, '_translentor_currency_order_rate', $currencies[$currency]['rate']);


                            update_post_meta($order_id, '_translentor_currency_order_base_currency', $this->default_currency);
                          
                            update_post_meta($order_id, '_translentor_currency_order_currency_changed_mannualy', time());
...
//for orders hook
    ...
    public function translentor_currency_order_metabox($post) {
        $data = array();
        $data['post'] = $post;
        $data['order'] = new WC_Order($post->ID);
        wp_enqueue_script('translentor_currency-meta-script', translentor_currency_LINK . 'js/meta-box.min.js', array('jquery'), translentor_currency_VERSION);
        echo $this->render_html(translentor_currency_PATH . 'views/translentor_currency_order_metabox.php', $data);
    }
    ...
    public function wp_head() {
        ...
        wp_enqueue_script( 'translentor-website-toast', translentor_currency_LINK . 'js/jquery.toast.min.js', translentor_currency_VERSION); 
        if (isset($_GET['currency'])) {
            wp_add_inline_script('woomentor-currency', $this->init_js_footer());
        }

        if ($this->shop_is_cached_preloader) {
            wp_add_inline_style('woomentor-currency', $this->add_css());
        }
        //+++
    ...
    public function add_css() {
        ob_start();
        ?>
        .translentor_currency_price_code.translentor_currency_preloader_ajax del,.translentor_currency_price_code.translentor_currency_preloader_ajax ins,.translentor_currency_price_code.translentor_currency_preloader_ajax span{
        display: none;
        }

        .translentor_currency_price_code.translentor_currency_preloader_ajax:after {
        content: " ";
        ...
        animation: translentor_currency_preloader_ajax 1.2s linear infinite;
        }
        @keyframes translentor_currency_preloader_ajax {
        0% {
    ...
    public function init_js_properties() {
        ...
        var translentor_currency_is_mobile = <?php echo (int) wp_is_mobile() ?>;
        var translentor_currency_special_ajax_mode = <?php echo 1 ?>;
        var translentor_currency_drop_down_view = "<?php echo $this->get_drop_down_view(); ?>";
        var translentor_currency_current_currency = <?php echo json_encode((isset($currencies[$this->current_currency]) ? $currencies[$this->current_currency] : $currencies[$this->default_currency])) ?>;
      
        var translentor_currency_redraw_cart = <?php echo apply_filters('translentor_currency_redraw_cart', 1) ?>;
        var translentor_currency_array_of_get = '{}';
        <?php if (!empty($_GET)): ?>
            <?php
//sanitization of $_GET array
            ...
            foreach ($_GET as $key => $value) {
                if (is_scalar($value)) {
                    $sanitized_get_array[sanitize_text_field($key)] = sanitize_text_field($value);
                }
            }
            ?>
            translentor_currency_array_of_get = '<?php echo str_replace("\\", "\\\\", str_replace("'", "", json_encode($sanitized_get_array))); ?>';
        <?php endif; ?>

        translentor_currency_array_no_cents = '<?php echo json_encode($this->no_cents); ?>';

        var translentor_currency_ajaxurl = "<?php echo admin_url('admin-ajax.php'); ?>";
        var translentor_currency_lang_loading = "<?php esc_html_e('loading', 'woomentor-currency') ?>";
        var translentor_currency_shop_is_cached =<?php echo (int) $this->shop_is_cached ?>;
        <?php
        return ob_get_clean();
    }
    ...
    public function woocommerce_settings_tabs_array($tabs) {
        $tabs['translentor_currency'] = esc_html__('Currency', 'woomentor-currency');
        return $tabs;
    }

    public function print_plugin_options() {

        if (isset($_POST['translentor_currency_name']) AND!empty($_POST['translentor_currency_name'])) {
            $result = array();
            update_option('translentor_currency_drop_down_view', sanitize_text_field($_POST['translentor_currency_drop_down_view']));
            update_option('translentor_currency_currencies_aggregator', sanitize_text_field($_POST['translentor_currency_currencies_aggregator']));
            update_option('translentor_currency_aggregator_key', sanitize_text_field($_POST['translentor_currency_aggregator_key']));

            update_option('translentor_currency_welcome_currency', sanitize_text_field($_POST['translentor_currency_welcome_currency']));
//***
            update_option('translentor_currency_is_multiple_allowed', (int) $_POST['translentor_currency_is_multiple_allowed']);
            update_option('translentor_currency_is_geoip_manipulation', (int) $_POST['translentor_currency_is_geoip_manipulation']);
            update_option('translentor_currency_collect_statistic', (int) $_POST['translentor_currency_collect_statistic']);
            update_option('translentor_currency_show_top_button', (int) $_POST['translentor_currency_show_top_button']);
            update_option('translentor_currency_is_fixed_user_role', (int) $_POST['translentor_currency_is_fixed_user_role']);

            update_option('translentor_currency_activate_page_list', sanitize_text_field($_POST['translentor_currency_activate_page_list']));
            update_option('translentor_currency_activate_page_list_reverse', (int) $_POST['translentor_currency_activate_page_list_reverse']);

            if ((int) $_POST['translentor_currency_is_multiple_allowed']) {
                update_option('translentor_currency_is_fixed_enabled', (int) $_POST['translentor_currency_is_fixed_enabled']);
                update_option('translentor_currency_is_fixed_coupon', (int) $_POST['translentor_currency_is_fixed_coupon']);
                update_option('translentor_currency_is_fixed_shipping', (int) $_POST['translentor_currency_is_fixed_shipping']);
                if ((int) $_POST['translentor_currency_is_fixed_enabled']) {
                    update_option('translentor_currency_force_pay_bygeoip_rules', (int) $_POST['translentor_currency_force_pay_bygeoip_rules']);
                } else {
                    update_option('translentor_currency_force_pay_bygeoip_rules', 0);
                }
            } else {
...
//***
            ...
            update_option('translentor_currency_special_ajax_mode', 1);
          
            //update_option('translentor_currency_use_curl', (int) $_POST['translentor_currency_use_curl']);
            update_option('wmc_storage', 'cookie');
            
            if (isset($_POST['translentor_currency_geo_rules'])) {
                $translentor_currency_geo_rules = array();
                if (!empty($_POST['translentor_currency_geo_rules'])) {
                    foreach ($_POST['translentor_currency_geo_rules'] as $curr_key => $countries) {
                        $translentor_currency_geo_rules[sanitize_text_field($curr_key)] = array();
                        if (!empty($countries)) {
                            foreach ($countries as $curr) {
                                $translentor_currency_geo_rules[sanitize_text_field($curr_key)][] = sanitize_text_field($curr);
                            }
                        }
                    }
                }
                update_option('translentor_currency_geo_rules', $translentor_currency_geo_rules);
            } else {
                update_option('translentor_currency_geo_rules', '');
            }

            //+++
            if (isset($_POST['translentor_currency_payment_control'])) {
                update_option('translentor_currency_payment_control', (int) sanitize_text_field($_POST['translentor_currency_payment_control']));
            } else {
                update_option('translentor_currency_payment_control', 0);
            }

            if (isset($_POST['translentor_currency_payments_rules'])) {
                $translentor_currency_payments_rules = array();
                if (!empty($_POST['translentor_currency_payments_rules'])) {
                    foreach ($_POST['translentor_currency_payments_rules'] as $payment_key => $currencies) {
                        $translentor_currency_payments_rules[sanitize_text_field($payment_key)] = array();
                        if (!empty($currencies)) {
                            foreach ($currencies as $curr) {
                                $translentor_currency_payments_rules[sanitize_text_field($payment_key)][] = sanitize_text_field($curr);
                            }
                        }
                    }
                }
                update_option('translentor_currency_payments_rules', $translentor_currency_payments_rules);
            } else {
                update_option('translentor_currency_payments_rules', '');
            }


            update_option('translentor_currency_hide_cents', (int) $_POST['translentor_currency_hide_cents']);
            update_option('translentor_currency_hide_on_front', 0);
            update_option('translentor_currency_rate_plus', (float) $_POST['translentor_currency_rate_plus']);
            update_option('translentor_currency_price_info', (int) $_POST['translentor_currency_price_info']);
            update_option('translentor_currency_no_cents', sanitize_text_field($_POST['translentor_currency_no_cents']));
            update_option('translentor_currency_restrike_on_checkout_page', (int) $_POST['translentor_currency_restrike_on_checkout_page']);
            update_option('translentor_currency_show_approximate_amount', (int) $_POST['translentor_currency_show_approximate_amount']);
            update_option('translentor_currency_show_approximate_price', (int) $_POST['translentor_currency_show_approximate_price']);
            update_option('translentor_currency_shop_is_cached', (int) $_POST['translentor_currency_shop_is_cached']);
            update_option('translentor_currency_shop_is_cached_preloader', (int) $_POST['translentor_currency_shop_is_cached_preloader']);
            update_option('translentor_currency_currency_version', WOOCOMMERCE_VERSION);
//***
            $cc = '';
...
//set default currency for all woocommerce system
            ...
            $this->init_currency_symbols();
//***
            $this->rate_auto_update = sanitize_text_field($_POST['translentor_currency_currencies_rate_auto_update']);
            $this->wp_cron_period = $this->get_translentor_currency_cron_schedules($this->rate_auto_update);
            $this->translentor_currency_wpcron_init(true);
...
//+++
    ...
    public function get_drop_down_view() {
        return apply_filters('translentor_currency_drop_down_view', get_option('translentor_currency_drop_down_view', 'ddslick'));
    }

    public function get_currencies($suppress_filters = false) {

        $currencies = get_option('translentor_currency', array());
        ...
        if (!$suppress_filters) {
            $currencies = apply_filters('translentor_currency_currency_data_manipulation', $currencies);
        }
    ...
    public function get_geo_rules() {
        return get_option('translentor_currency_geo_rules', array());
    }

    public function is_use_geo_rules() {
        //$is = get_option('translentor_currency_use_geo_rules', 0);
    ...
    public function woocommerce_currency_symbol($currency_symbol) {
        ...
        if (!isset($wp_query)) {
            if (function_exists('is_account_page') AND ( is_order_received_page() || is_account_page())) {
                if (apply_filters('translentor_currency_currency_symbol_on_order', false)) {
                ...
                if (!empty($values)) {
                    foreach ($values as $product_id => $price) {
                        ...
                        if ($this->is_fixed_enabled AND $this->fixed->is_exists($product_id, $current_currency, $type)) {
                            ...
                            if ((int) $tmp !== -1) {
                                if (wc_tax_enabled()) {
                                    $tmp = $this->translentor_currency_calc_tax_price(wc_get_product($product_id), $tmp);
                                }
                        ...
                        if (!$is_price_custom) { {
                                if (wc_tax_enabled()) {
                                    // $price = $this->translentor_currency_calc_tax_price(wc_get_product($product_id), $price);
                                }
                                $prices_array[$key][$product_id] = apply_filters('translentor_currency_woocommerce_variation_prices', number_format(floatval($this->translentor_currency_exchange_value(floatval($price))), $precision, $this->decimal_sep, ''));
    ...
    public function raw_woocommerce_price($price, $product = NULL, $min_max = NULL, $display = NULL) {
        ...
        if (isset($_REQUEST['translentor_currency_block_price_hook'])) {
            return $price;
        }

        //***

        if (isset($_REQUEST['translentor_currency_raw_woocommerce_price_currency'])) {
            $this->current_currency = sanitize_text_field($_REQUEST['translentor_currency_raw_woocommerce_price_currency']);
        }
        $currencies = $this->get_currencies();
...
//***
        ...
        if ($this->is_fixed_enabled) {
            if ($this->is_multiple_allowed AND $product !== NULL AND is_object($product)) {
                ...
                if ((int) $tmp_val !== -1) {
                    $price = apply_filters('translentor_currency_fixed_raw_woocommerce_price', $tmp_val, $product, $price);
                    $is_price_custom = true;
                }
            }
        }
        ...
        return apply_filters('translentor_currency_raw_woocommerce_price', $price);
            ...
            if (!empty($cart->cart_contents)) {
                ...
                foreach ($cart->cart_contents as $ci) {
                    if ($ci['variation_id'] > 0) {
                        $cart_subtotal += $this->translentor_currency_exchange_value($ci['line_total']);
                    } else {
        ...
        if ($product !== NULL AND is_object($product)) {
            ...
            if (isset($product_geo_data[$type . '_price_geo'])) {
                if (!empty($product_geo_data[$type . '_price_geo'])) {
                    $user_country = $this->storage->get_val('translentor_currency_user_country');
    ...
    public function get_welcome_currency() {
        return get_option('translentor_currency_welcome_currency');
    }

    public function get_customer_signs() {
        $signs = array();
        $data = get_option('translentor_currency_customer_signs', '');
        if (!empty($data)) {
            $data = explode(',', $data);
    ...
    public function force_pay_bygeoip_rules() {
        //$use_geo_rules = get_option('translentor_currency_use_geo_rules', 0);
        $use_geo_rules = true;
        if ($this->is_multiple_allowed AND $this->force_pay_bygeoip_rules AND $use_geo_rules) {
            $country = $this->storage->get_val('translentor_currency_user_country');
            $user_currency = $this->get_currency_by_country($country);
            if (!empty($user_currency)) {
                //$user_currency is empty its mean that current country is not in geo ip rules
                $this->set_currency($user_currency);
            }
            do_action('translentor_currency_force_pay_bygeoip_rules', $country, $user_currency, $this->current_currency);
        }
    }
    ...
    public function woocommerce_price_format() {
        ...
        return apply_filters('translentor_currency_price_format', $format, $currency_pos);
    }

//[translentor_currency]
    public function translentor_currency_shortcode($args) {
        if (empty($args)) {
            $args = array();
        }
        ...
        if (isset($args['sd']) AND intval($args['sd']) > 0) {
            ...
            global $translentor_currency_SD;
            $args['sd_id'] = intval($args['sd']);
            $args['sd_settings'] = $translentor_currency_SD->get(intval($args['sd']));
        }

        return $this->render_html(translentor_currency_PATH . 'views/shortcodes/translentor_currency.php', $args);
    }

    //[translentor_currency_price] from v.2.3.3/1.3.3
    public function translentor_currency_price_shortcode($args) {
        $price = "";
        $product_o = null;
        ...
        return apply_filters('translentor_currency_price_shortcode', $price, $product_o);
    }

//[translentor_currency_converter exclude="GBP,AUD" precision=2]
    public function translentor_currency_converter($args) {
        if (empty($args)) {
            $args = array();
        }
        return $this->render_html(translentor_currency_PATH . 'views/shortcodes/translentor_currency_converter.php', $args);
    }

//[translentor_currency_rates exclude="GBP,AUD" precision=2]
    public function translentor_currency_rates($args) {
        if (empty($args)) {
            $args = array();
        }
        return $this->render_html(translentor_currency_PATH . 'views/shortcodes/translentor_currency_rates.php', $args);
    }

//[translentor_currency_show_current_currency text="" currency="" flag=1 code=1]
    public function translentor_currency_show_current_currency($atts) {
        $currencies = $this->get_currencies();
        extract(shortcode_atts(array(
            'text' => esc_html__('Current currency is:', 'woomentor-currency'),
        ...
        $args['flag'] = $flag;
        $args['code'] = $code;
        return $this->render_html(translentor_currency_PATH . 'views/shortcodes/translentor_currency_show_current_currency.php', $args);
    }

//[translentor_currency_show_custom_price value=20] -> value should be in default currency
    public function translentor_currency_show_custom_price($atts) {
        extract(shortcode_atts(array('value' => 0), $atts));
        $currencies = $this->get_currencies();
        $convert = true;
        $_REQUEST['translentor_currency_show_custom_price'] = TRUE;
        ...
        unset($_REQUEST['translentor_currency_show_custom_price']);
        return '<span class="translentor_currency_amount_custom_price" data-value="' . $value . '">' . $wc_price . "</span>";
    }

    //for geo ip demo
    public function translentor_currency_geo_hello($atts = '') {
        ...
        if (!empty($pd) AND!empty($countries)) {
            $text = '<span class="translentor_currency_geo_hello">' . sprintf(esc_html__('Your country is: %s. (defined by woocommerce GeoIP functionality)', 'woomentor-currency'), $countries[$pd['country']]) . '</span>';
        } else {
            $text = '<i class="translentor_currency_geo_hello_not">' . esc_html__('Your country is not defined! Troubles with GeoIp service.', 'woomentor-currency') . '</i>';
        }
...
//+++ get curr updated values back
        $request = array();
        $this->default_currency = strtoupper(sanitize_text_field($_REQUEST['currency_name']));
        $_REQUEST['no_ajax'] = TRUE;
        foreach ($currencies as $key => $currency) {
...
//order data registration
    public function woocommerce_thankyou_order_id($order_id) {
        $currencies = $this->get_currencies();
//+++
        update_post_meta($order_id, '_translentor_currency_order_rate', $currencies[$this->current_currency]['rate']);
        //wc_add_order_item_meta($order_id, '_translentor_currency_order_rate', $currencies[$this->current_currency]['rate'], true);

        update_post_meta($order_id, '_translentor_currency_order_base_currency', $this->default_currency);
        //wc_add_order_item_meta($order_id, '_translentor_currency_order_base_currency', $this->default_currency, true);

        update_post_meta($order_id, '_translentor_currency_order_currency_changed_mannualy', 0);
        // wc_add_order_item_meta($order_id, '_translentor_currency_order_currency_changed_mannualy', 0, true);
...
//***
        ...
        $this->current_currency = $this->default_currency;
        $value .= esc_html__('Total in basic currency: ', 'woomentor-currency') . $this->wc_price($amount, false, array('currency' => $this->default_currency));
...
//***************************** email actions
    ...
    public function woocommerce_email_actions($email_actions) {
        $_REQUEST['translentor_currency_order_emails_is_sending'] = 1;
        if (isset($_REQUEST['translentor_currency_in_order_currency'])) {
            $this->current_currency = sanitize_text_field($_REQUEST['translentor_currency_in_order_currency']);
...
//for hook woocommerce_paypal_args
    ...
    public function woocommerce_price_html($price_html, $product) {
        ...
        if ($customer_price_format === -1) {
            $customer_price_format = get_option('translentor_currency_customer_price_format', '__PRICE__');
        }
...
//+++
        ...
        if (!empty($customer_price_format)) {
            ...
            $classes = "translentor_currency_price_code";
            if ($this->shop_is_cached AND $this->shop_is_cached_preloader) {
                $classes .= " translentor_currency_preloader_ajax";
            }
            ...
            $price_html = apply_filters('translentor_currency_price_html_tail', $price_html);
        }
...
//add additional info in price html
        if (get_option('translentor_currency_price_info', 0) AND!(is_admin() AND!isset($_REQUEST['get_product_price_by_ajax'])) AND!isset($_REQUEST['hide_translentor_currency_price_info_list'])) {
            $info = "<ul class='translentor_currency_price_info_list'>";
            $current_currency = $this->current_currency;
            ...
            foreach ($currencies as $сurr) {
                ...
                if ($product_type == 'variable') {
                    ...
                    if (wc_tax_enabled()) {
                        $min_value = $this->translentor_currency_calc_tax_price($product, $min_value);
                        $max_value = $this->translentor_currency_calc_tax_price($product, $max_value);
                    }
                    //+++
                    $_REQUEST['translentor_currency_wc_price_convert'] = FALSE;
                    ...
                    unset($_REQUEST['translentor_currency_wc_price_convert']);
                    ...
                    foreach ($child_ids as $prod_id) {
                        ...
                        if ($product_type1 == 'variable') {
                            ...
                            if (wc_tax_enabled()) {
                                $prices[] = $this->translentor_currency_calc_tax_price($product1, $min_value);
                                $prices[] = $this->translentor_currency_calc_tax_price($product1, $max_value);
                            } else {
                                $prices[] = $min_value;
                                $prices[] = $max_value;
                            }
                            ...
                            if ($this->is_fixed_enabled AND $this->is_multiple_allowed) {
                                ...
                                if ($is_exists AND!$is_empty) {
                                    ...
                                    if (floatval($this->fixed->get_value($prod_id, $сurr['name'], $type)) > 0) {
                                        ...
                                        if (wc_tax_enabled()) {
                                            $prices[] = $this->translentor_currency_calc_tax_price($product1, floatval($this->fixed->get_value($prod_id, $сurr['name'], $type)));
                                        } else {
                                    ...
                                    if (wc_tax_enabled()) {
                                        $prices[] = $this->translentor_currency_calc_tax_price($product1, $product1->get_price('edit') * $currencies[$сurr['name']]['rate']);
                                    } else {
                    ...
                    if (wc_tax_enabled()) {
                        $value = $this->translentor_currency_calc_tax_price($product, $value);
                    }
            ...
            $info .= "</ul>";
            $info = '<div class="translentor_currency_price_info"><span class="translentor_currency_price_info_icon"></span>' . $info . '</div>';
        ...
        if (get_option('translentor_currency_show_approximate_price', 0) AND!is_admin()) {
            $price_html = $this->translentor_currency_add_approx_to_price($price_html, $product);
        }
        ...
        if ($this->is_multiple_allowed) {
            if (is_object($coupon) AND method_exists($coupon, 'is_type')) {
                if (!$coupon->is_type(array('percent_product', 'percent'))) {
                    $discount = $this->translentor_currency_exchange_value(floatval($discount));
                }
            }
        }
    ...
    public function woocommerce_coupon_validate_minimum_amount($is, $coupon) {
        ...
        if ($this->current_currency != $this->default_currency AND get_option('translentor_currency_is_multiple_allowed', 0)) {
            $currencies = $this->get_currencies();
        ...
        if ($this->current_currency != $this->default_currency) {
            ...
            switch ($err_code) {
                case 112:
                    ...
                    $err = sprintf(esc_html__('The maximum spend for this coupon is %s.', 'woomentor-currency'), wc_price($amount));
                    break;

                case 108:
                    $amount = $coupon->get_minimum_amount() * $rate;
                    $err = sprintf(esc_html__('The minimum spend for this coupon is %s.', 'woomentor-currency'), wc_price($amount));
                    break;
...
//wp filter for values which is in basic currency and no possibility do it automatically
    public function translentor_currency_exchange_value($value) {
        $currencies = $this->get_currencies();
...
//set it to default
    ...
    public function set_currency($currency = '') {
        ...
        $this->current_currency = $currency;
        //setcookie('translentor_currency_current_currency', $currency, time() + 1 * 24 * 3600);
        //$this->storage->set_val('translentor_currency_current_currency', $currency);
    }
...
//https://wordpress.org/support/topic/vat-values-are-not-switched-to-another-currency-for-shipping
    ...
    public function wcml_raw_price_amount($value) {
        return $this->translentor_currency_exchange_value($value);
    }

//ajax
    public function translentor_currency_convert_currency() {
        $currencies = $this->get_currencies();
...
//for refreshing mini-cart widget
    public function woocommerce_before_mini_cart() {
        ...
        $_REQUEST['translentor_currency_woocommerce_before_mini_cart'] = 'mini_cart_refreshing';
        WC()->cart->calculate_totals();
    }

//for refreshing mini-cart widget
    public function woocommerce_after_mini_cart() {
        unset($_REQUEST['translentor_currency_woocommerce_before_mini_cart']);
    }

//ajax
    public function translentor_currency_rates_current_currency() {
        wp_die(do_shortcode('[translentor_currency_rates exclude="' . sanitize_text_field($_REQUEST['exclude']) . '" precision="' . sanitize_text_field($_REQUEST['precision']) . '" current_currency="' . sanitize_text_field($_REQUEST['current_currency']) . '"]'));
    }

    public function wc_price($price, $convert = true, $args = array(), $product = NULL, $decimals = -1) {
        if (!isset($_REQUEST['translentor_currency_wc_price_convert'])) {
            $_REQUEST['translentor_currency_wc_price_convert'] = true;
        }
...
//***
        $currencies = $this->get_currencies();
        if (isset($currencies[$currency])/* AND !isset($_REQUEST['translentor_currency_show_custom_price']) */) {
...
//***
                    ...
                    if ($this->is_fixed_enabled) {
                        ...
                        if ($is_exists AND!$is_empty) {
                            ...
                            if (floatval($this->fixed->get_value($product_id, $currency, $type)) > 0) {
                                ...
                                if (wc_tax_enabled()) {
                                    $price = $this->translentor_currency_calc_tax_price($product, $price);
                                }
                            }
                        }
                    }
        ...
        //***
        $unformatted_price = 0;
        if ($convert AND $_REQUEST['translentor_currency_wc_price_convert'] AND!$is_price_custom) {
            $price = $this->raw_woocommerce_price(floatval($negative ? $price * -1 : $price));
        ...
        $return = '<span class="translentor_currency_amount">' . $formatted_price . '</span>';
    ...
    public function woocommerce_available_variation($variation_data, $product, $variation) {
        ...
        $_REQUEST['translentor_currency_woocommerce_available_variation_is'] = TRUE;
        add_filter('raw_woocommerce_price', array($this, 'raw_woocommerce_price'), 9999);
        ...
        $_REQUEST['hide_translentor_currency_price_info_list'] = true;
        $variation_data = apply_filters('translentor_currency_woocommerce_available_variation', array_merge($variation->get_data(), array(
        ...
        unset($_REQUEST['hide_translentor_currency_price_info_list']);
        unset($_REQUEST['translentor_currency_woocommerce_available_variation_is']);
        remove_filter('raw_woocommerce_price', array($this, 'raw_woocommerce_price'), 9999);

        return apply_filters('translentor_currency_woocommerce_available_variation', $variation_data, $product, $variation);
    }
...
//for price redrawing on front if site using cache plugin functionality
    public function translentor_currency_get_products_price_html() {
        $result = array();
...
//***
    ...
    public function translentor_currency_get_variation_products_price_html() {
        $result = array();
...
//***
    ...
    function translentor_currency_get_custom_price_html() {
        $result = array();
...
//***
            ...
            if (!empty($custom_prices) AND is_array($custom_prices)) {
                ...
                foreach ($custom_prices as $p) {
                    $result[$p] = do_shortcode("[translentor_currency_show_custom_price value=" . sanitize_text_field($p) . "]");
                }
            }
        }
...
//count amount in basic currency from any currency
    ...
    public function recalculate_order($order_id, $selected_currency = '') {
        ...
        $_translentor_currency_order_rate = get_post_meta($order_id, '_translentor_currency_order_rate', true);
        ...
        update_post_meta($order_id, '_translentor_currency_order_currency', $selected_currency);
        update_post_meta($order_id, '_order_currency', $selected_currency);
        update_post_meta($order_id, '_translentor_currency_order_base_currency', $this->default_currency);
        update_post_meta($order_id, '_translentor_currency_order_rate', floatval($currencies[$selected_currency]['rate']));
        update_post_meta($order_id, '_translentor_currency_order_currency_changed_mannualy', time());

        //***

        $_order_shipping = get_post_meta($order_id, '_order_shipping', true);
        $val = $this->back_convert($_order_shipping, $_translentor_currency_order_rate, $decimals);
        ...
        $val = $this->back_convert($_order_total, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
        ...
        $val = $this->back_convert($_refund_amount, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
        ...
        $val = $this->back_convert($_cart_discount_tax, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
        ...
        $val = $this->back_convert($_order_tax, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
        ...
        $val = $this->back_convert($_order_shipping_tax, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
        ...
        $val = $this->back_convert($_cart_discount, $_translentor_currency_order_rate, $decimals);
        if ($selected_currency !== $this->default_currency) {
            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
        }
...
//***
        ...
        $get_items_sql = $this->translentor_currency_prepare("SELECT order_item_id,order_item_type FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d ", $data_sql);
            ...
            foreach ($line_items as $v) {
                ...
                switch ($order_item_type) {
                    case 'line_item':
                        ...
                        $val = $this->back_convert($amount, $_translentor_currency_order_rate, $decimals);
                        if ($selected_currency !== $this->default_currency) {
                            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
                        }
                                ...
                                if (!empty($values)) {
                                    if (is_array($values)) {
                                        foreach ($values as $k => $value) {
                                            if (is_numeric($value)) {
                                                $_line_tax_data[$key][$k] = $this->back_convert($value, $_translentor_currency_order_rate, $decimals);
                                                if ($selected_currency !== $this->default_currency) {
                                        ...
                                        if (is_numeric($values)) {
                                            $_line_tax_data[$key] = $this->back_convert($values, $_translentor_currency_order_rate, $decimals);
                                            if ($selected_currency !== $this->default_currency) {
                            ...
                            foreach ($taxes as $key => $values) {
                                if (!empty($values)) {
                                    if (is_array($values)) {
                                        foreach ($values as $k => $value) {
                                            if (is_numeric($value)) {
                                                $taxes[$key][$k] = $this->back_convert($value, $_translentor_currency_order_rate, $decimals);
                                                if ($selected_currency !== $this->default_currency) {
                                        ...
                                        if (is_numeric($values)) {
                                            $taxes[$key] = $this->back_convert($values, $_translentor_currency_order_rate, $decimals);
                                            if ($selected_currency !== $this->default_currency) {
                    ...
                    case 'tax':
                        ...
                        $val = $this->back_convert($amount, $_translentor_currency_order_rate, 3);
                        if ($selected_currency !== $this->default_currency) {
                            $val = floatval($val) * floatval($currencies[$selected_currency]['rate']);
                        }
...
//ajax
    public function translentor_currency_recalculate_order_data() {
    ...
    public function translentor_currency_recalculate_orders_data() {
        if (!current_user_can('manage_options')) {
            return;
        }
        $orders = array();
...
//wp-content\plugins\woocommerce\templates\cart\cart.php
    public function woocommerce_cart_item_price($product_price, $cart_item, $cart_item_key) {
        $user_currency = $this->get_currency_by_country($this->storage->get_val('translentor_currency_user_country'));
        $currencies = $this->get_currencies();
...
//wp-content\plugins\woocommerce\templates\cart\cart-totals.php
    public function woocommerce_cart_subtotal($cart_subtotal, $compound, $data) {
        $user_currency = $this->get_currency_by_country($this->storage->get_val('translentor_currency_user_country'));
        $currencies = $this->get_currencies();
        ...
        if ($user_currency != $this->current_currency AND!empty($user_currency)) {
            ...
            $amount = 0;
            if ($compound) {
                $amount = $data->cart_contents_total + $data->shipping_total + $data->get_taxes_total(false, false);
// Otherwise we show cart items totals only (before discount)
            } else {

// Display varies depending on settings
                if ($this->get_cart_tax_mode($data) == 'excl') {
                    $amount = $data->subtotal_ex_tax;
                } else {
...
//***
    ...
    public function woocommerce_cart_tax_totals($tax_totals, $data) {
...
//***
    ...
    public function translentor_currency_add_approx_to_price($price_html, $product) {
        $user_currency = $this->get_currency_by_country($this->storage->get_val('translentor_currency_user_country'));
        $currencies = $this->get_currencies();
                        ...
                        if ($this->is_fixed_enabled AND $this->is_multiple_allowed) {
                            ...
                            if ($is_exists AND!$is_empty) {
                                ...
                                if (floatval($this->fixed->get_value($prod_id, $user_currency, $type)) > 0) {
                                    ...
                                    if (wc_tax_enabled()) {
                                        $prices[] = $this->translentor_currency_calc_tax_price($product1, floatval($this->fixed->get_value($prod_id, $user_currency, $type)));
                                    } else {
                                ...
                                if (wc_tax_enabled()) {
                                    $prices[] = $this->translentor_currency_calc_tax_price($product1, $product1->get_price('edit') * $currencies[$user_currency]['rate']);
                                } else {
...
//***
    ...
    private function get_cart_item_price_html($wc_price) {
        $html = '<div class="translentor_currency_cart_item_price">';
        $html .= apply_filters('translentor_currency_get_approximate_amount_text', sprintf(esc_html__('(Approx. %s)', 'woomentor-currency'), $wc_price), $wc_price);
        $html .= '</div>';
        return $html;
    }
...
//as it recounted twice - down it!
    ...
    public function translentor_currency_fix_decimals($code) {
        global $wp_filter;
    ...
    function translentor_currency_woocommerce_cart_price_html($price_html, $product = null) {
        ...
        if (!empty($customer_price_format)) {
            $txt = '<span class="translentor_currency_special_price_code" >' . $customer_price_format . '</span>';
            $txt = str_replace('__PRICE__', $price_html, $txt);
            $price_html = str_replace('__CODE__', $this->current_currency, $txt);
            $price_html = apply_filters('translentor_currency_price_html_tail', $price_html);
        }
    ...
    public function translentor_currency_prepare($query, $args) {
        if (is_null($query)) {
            return;
        }
        $sql_val = array();
        ...
        foreach ($args as $item) {
            ...
            $sql_val[] = $this->translentor_currency_escape_sql($item['type'], $item['val']);
        }
        return @vsprintf($query, $sql_val);
    }

    public function translentor_currency_escape_sql($type, $value) {
        switch ($type) {
            case'string':
                global $wpdb;
    ...
    public function check_currency_on_checkout() {
        if (!$this->is_multiple_allowed) {
            ...
            //setcookie('translentor_currency_current_currency', $curr_curr, time() + 1 * 24 * 3600);
            //$this->storage->set_val('translentor_currency_current_currency', $curr_curr);
        }
    }

    //compatibilites
    function translentor_currency_convert_price($amount, $is_cond_multi = false) {
        if (get_option('translentor_currency_is_multiple_allowed', 0) AND $is_cond_multi) {
            return $this->translentor_currency_exchange_value($amount);
    ...
    function translentor_currency_back_convert_price($amount, $is_cond_multi = false) {
        $currencies = $this->get_currencies();
    ...
    function translentor_currency_convert_price_wcdp($amount, $is_cond_multi = false, $method = '') {
    ...
    function woocommerce_coupon_loaded($coupon) {
        ...
        foreach ($prices as $key => $val) {
            if (!('amount' == $key AND!$convert)) {
                $prices[$key] = $this->translentor_currency_exchange_value($val);
            }
    ...
    function translentor_currency_calc_tax_price($product, $price) {
    ...
    function translentor_currency_before_calculate_totals_geoip_fix() {
    ...
    public function prepare_default_currencies() {
        ...
        switch ($wc_currency) {
            ...
            default :
                ...
                $default['USD']['rate'] = 1.15;
                $default['USD']['description'] = esc_html__('change the rate and this description to the right values', 'woomentor-currency');
                break;
        }
    ...
    public function translentor_currency_all_order_ids() {
    ...
    function translentor_currency_filter_gateways($gateway_list) {
        global $translentor_currency;
        if (is_checkout() OR is_checkout_pay_page()) {
            $exclude = get_option('translentor_currency_payments_rules', array());
            if (!is_array($exclude)) {
                $exclude = array();
            }
            foreach ($exclude as $gateway_key => $currencies) {
                $behavior = true;
                $behavior = in_array($translentor_currency->current_currency, $currencies);
                if (get_option('translentor_currency_payment_control', 0)) {
                    $behavior = !$behavior;
                }
    ...
    function manage_posts_extra_tablenav($width) {
        ...
        if (get_option('translentor_currency_is_multiple_allowed', 0)) {
            if (function_exists("wc_get_order_types") AND in_array($typenow, wc_get_order_types('order-meta-boxes'), true) AND $width == 'top') {
                ?>
                <a href="javascript:translentor_currency_recalculate_all_orders_data();void(0);" class="button translentor_currency_recalculate_all_orders_curr_button"><?php esc_html_e("Recalculate all orders", 'woomentor-currency') ?>&nbsp;<img class="help_tip" data-tip="translentor_currency: <?php esc_html_e('Recalculate all orders with basic currency. Recommended test this option on the clone of your site! Read the documentation of the plugin about it!', 'woomentor-currency') ?>" src="<?php echo translentor_currency_LINK ?>/img/help.png" height="16" width="16" /><img class="translentor_currency_ajax_preload" src="<?php echo translentor_currency_LINK ?>/img/loading_large.gif" height="18" width="18" /></a>
                <?php
            }
        }
    }
    ...
    public function structured_data_product_offer($markup_offer, $product) {
        global $translentor_currency;
        $current = $translentor_currency->current_currency;
        $rate = 1;
        if ($current != $translentor_currency->default_currency) {
            $currencies = $translentor_currency->get_currencies();
            $rate = $currencies[$current]['rate'];
        }
        $precision = $translentor_currency->get_currency_price_num_decimals('EUR', $translentor_currency->price_num_decimals);

        if (isset($markup_offer["priceSpecification"])) {
            $markup_offer["priceSpecification"]["priceCurrency"] = $translentor_currency->default_currency;
        }
        if (isset($markup_offer["priceCurrency"])) {
            $markup_offer["priceCurrency"] = $translentor_currency->default_currency;
        }

        if ($translentor_currency->is_multiple_allowed) {
    ...
    public function translentor_currency_set_currency_ajax() {
        if (isset($_REQUEST['currency']) AND!$this->is_currency_private($_REQUEST['currency'])) {
            $currency = sanitize_text_field($_REQUEST['currency']);
            $this->set_currency($currency);
            $this->statistic->register_switch(strtoupper(sanitize_text_field($currency)), strtoupper($this->storage->get_val('translentor_currency_user_country')));
        }
    }

    public function init_marketig_translentor_currency() {
        $alert = new translentor_currency_ADV();
        $alert->init();
    }
    ...
    public function delete_profiles_data() {
        ...
        $response = array();
        if (!$key) {
            $response['info'] = esc_html__("No such profile", 'woomentor-currency');
        } else {
            $this->geoip_profiles->delete_data($key);
            $response['info'] = esc_html__("Profile is deleted", 'woomentor-currency');
        }
    ...
    public function get_decimal_sep($currency, $value = '.') {
        ...
        $value = apply_filters('translentor_currency_price_decimal_sep', $value, $currency);
    ...
    public function get_thousand_sep($currency, $value = ',') {
        ...
        $value = apply_filters('translentor_currency_price_thousand_sep', $value, $currency);
        return $value;
    }

    public function update_profiles_data() {

        if (!(isset($_POST['translentor_currency_wpnonce_geo']) && wp_verify_nonce($_POST['translentor_currency_wpnonce_geo'], 'translentor_currency_wpnonce_geo'))) {
        ...
        $title = esc_html__("New profile", 'woomentor-currency');
        ...
        if ($key) {
            ...
            $info = esc_html__("is updated!", 'woomentor-currency');
        } else {
            $key = $this->geoip_profiles->add_data($value);

            $info = esc_html__("is added!", 'woomentor-currency');
        }
        ...
        ob_start();
        ?>
        <option data-key="<?php echo esc_html($key) ?>" value='<?php echo json_encode($countries) ?>' ><?php echo esc_html($title) ?></option>
        <?php
