...
class translentor_elementor_widget extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		$this->add_render_attribute(
			...
			array(
				'class' => array( 'progress-container_footer', 'fixed-top' ),
			)
		);
		$this->add_render_attribute(
			'progress-container',
			array(
				'class' => array( 'progress-container', 'fixed-top' ),
			)
		);
		$this->add_render_attribute(
			'cttopbarlist',
			array(
				'class' => array( 'list-unstyled', 'list-inline', 'ct-topbar__list' ),
			)
		);
		$this->add_render_attribute(
			'progress-bar_footer',
	...
	<div <?php echo $this->get_render_attribute_string( 'container' ); ?>>
		<ul <?php echo $this->get_render_attribute_string( 'cttopbarlist' ); ?>>
			...
			if ( $settings['icon_position'] === 'right' ) {
				...
				} elseif ( $settings['nav_style_location'] === 'left' ) {
					$location         = 'right: 0;';
		...
		text-align: center;';
					...
					$class            = 't-footer-center-side';
							...
							$this->add_render_attribute(
								...
								array(
									'id'    => 'mydata_footer',
									'class' => array( 'list-unstyled', 'language_footer', 'vertical' ),
									'style' => array( 'display: grid;', 'float: ' . esc_html( $settings['nav_style_location'] ) . ' !important;', esc_html( $position ) . ';' ),
			...
			<span class="text-label"><?php echo esc_html( $settings['translentor_text'] ); ?></span>
					<?php } \Elementor\Icons_Manager::render_icon( $settings['translentor_icon'], array( 'aria-hidden' => 'true' ) ); ?>
				</span>
					<?php
			} else {
				if ( $settings['nav_style_location'] === 'right' ) {
		...
		text-align: center;';
				...
				?>
			<li <?php echo $this->get_render_attribute_string( 'ct-language' ); ?> id="<?php echo $class; ?>" style="<?php echo $location; ?>"><span
					<?php echo $this->get_render_attribute_string( 'drop_footer' ); ?>><?php \Elementor\Icons_Manager::render_icon( $settings['translentor_icon'], array( 'aria-hidden' => 'true' ) ); ?>
					<?php if ( $settings['translentor_text'] != '' ) { ?>
				...
				foreach ( $settings['language_settings'] as $key => $item ) {
					...
					if ( ! in_array( $item['choose_language'], $selected ) ) {
						...
						$this->add_render_attribute(
							'footer_googlelink' . $i,
							array(
								'class'     => array( 'lang-es', 'lang-select_footer' ),
						...
						<a 
								<?php
								echo wp_kses_post( $this->get_render_attribute_string( $repeater_setting__country ) );
...
<div <?php echo $this->get_render_attribute_string( 'ct-topbar' ); ?>>
	<div <?php echo $this->get_render_attribute_string( 'container' ); ?>>
				...
				if ( $settings['nav_style_location'] === 'right' ) {
					...
					$class                = 't-center-side';
							...
							$this->add_render_attribute(
								...
								array(
									'id'    => 'mydata',
									'class' => array( 'list-unstyled', 'ct-language__dropdown', 'vertical' ),
									'style' => array( 'display: grid;', 'float: ' . esc_html( $settings['nav_style_location'] ) . ' !important;', esc_html( $position ) . ';' ),
		...
		text-align: center;';
				...
				?>
			<li <?php echo $this->get_render_attribute_string( 'ct-language' ); ?> id="<?php echo $class; ?>" style="<?php echo $location; ?>"><span
					<?php echo $this->get_render_attribute_string( 'drop' ); ?>><?php \Elementor\Icons_Manager::render_icon( $settings['translentor_icon'], array( 'aria-hidden' => 'true' ) ); ?>
					<?php if ( $settings['translentor_text'] != '' ) { ?>
			<span class="text-label">
				...
				foreach ( $settings['language_settings'] as $key => $item ) {
					...
					if ( ! in_array( $item['choose_language'], $selected ) ) {
							...
							$this->add_render_attribute(
								'googlelink' . $i,
								array(
									'class'     => array( 'lang-es', 'lang-select' ),
									'href'      => '#googtrans(en|' . $code . ')',
									'data-lang' => $code,
								)
							);
