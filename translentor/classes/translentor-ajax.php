...
class translentor_Ajax{
    ...
    /**
     * translentor_Ajax constructor.
     *
     * Establishes db connection and triggers function to output translations.
     */
    public function __construct( ){

        if ( !isset( $_POST['action'] ) || $_POST['action'] !== 'translentor_get_translations_regular' || empty( $_POST['originals'] ) || empty( $_POST['language'] ) || empty( $_POST['original_language'] ) ) {
            
            die();
        }

        include './translentor-external-functions.php';
        if ( !translentor_is_valid_language_code( $_POST['language'] ) || !translentor_is_valid_language_code( $_POST['original_language'] ) ) {//phpcs:ignore
            echo json_encode( 'translentor Error: Invalid language code' );
            exit;
        }
     ...
     * Sanitize strings.
     */
    protected function sanitize_strings( $strings){
        ...
        if ( is_array( $strings ) ) {
            foreach ($strings as $key => $string) {
	            $strings[$key] = mysqli_real_escape_string( $this->connection, translentor_full_trim( $string, array( 'numerals'=> $numerals_option ) ) );
            }
        }
        return $strings;
    }
    ...
    /**
     * connect to database using wp-config
     */
...
//get credentials..
    ...
    protected function output_translations( $strings, $skip_machine_translation, $language, $original_language ){
        $sql = 'SELECT original, translated, status FROM ' . $this->table_prefix . 'translentor_dictionary_' . strtolower( $original_language ) . '_' . strtolower( $language ) . ' WHERE original IN (\'' . implode( "','", $strings ) .'\') AND status != 0';
        $result = mysqli_query( $this->connection, $sql );
        if ( $result === false ){
            $this->return_error();
        }else {
	        ...
	        $dictionary_by_original = translentor_sort_dictionary_by_original( $dictionaries, 'regular', 'dynamicstrings', null, null );
