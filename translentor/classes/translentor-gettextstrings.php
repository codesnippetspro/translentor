...
class translentor_Editor_Api_Gettext_Strings {

	/* @var Lang_Query */
	protected $lang_query;
	/* @var translentor_SP_Slug_Manager*/
	...
	/**
	 * Hooked to wp_ajax_translentor_get_translations_gettext
	 */
	public function gettext_get_translations(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			if (isset($_POST['action']) && $_POST['action'] === 'translentor_get_translations_gettext' && !empty($_POST['string_ids']) && !empty($_POST['language']) && in_array($_POST['language'], get_option('translentor_settings_elementor')['translentor_lang'])) {
				...
				if ( is_array( $gettext_string_ids ) ) {
					...
					$dictionaries[$current_language] = $this->lang_query->get_gettext_string_rows_by_ids( $gettext_string_ids, $current_language );
					...
					foreach (get_option('translentor_settings_elementor')['translentor_lang'] as $language) {
						...
						if( !empty( $original_strings ) && !empty( $lang_original_string_details ) ){
							$dictionaries[$language] = $this->lang_query->get_gettext_string_rows_by_original( $original_strings, $language );
							...
							if( !empty( $lang_original_string_details ) ){
								foreach( $lang_original_string_details as $lang_original_string_detail ){
									...
									$db_id = $this->lang_query->insert_gettext_strings( array( array('original' => $lang_original_string_detail['original'], 'translated' => $translated, 'domain' => $lang_original_string_detail['domain']) ), $language );
				...
				$dictionary_by_original = translentor_sort_dictionary_by_original( $dictionaries, 'gettext', $localized_text['gettextstrings'], sanitize_text_field( $_POST['language'] ) );
				die( translentor_safe_json_encode( $dictionary_by_original ) );//phpcs:ignore
	 ...
	 */
	public function translentor_save_translations(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( apply_filters( 'translentor_translating_capability', 'manage_options' ) ) ) {
			if (isset($_POST['action']) && $_POST['action'] === 'translentor_save_translations_gettext' && !empty($_POST['strings'])) {
				check_ajax_referer( 'translentor_save_translations', 'security' );
				...
				foreach( $update_strings as $language => $update_string_array ) {
					$this->lang_query->update_gettext_strings( $update_string_array, $language, array('id','translated', 'status') );
                    $this->lang_query->remove_possible_duplicates($update_string_array, $language, 'gettext');
				}

                do_action('translentor_save_editor_translations_gettext_strings', $update_strings, $this->settings);
			}
		}
