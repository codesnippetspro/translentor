...
class translentor_Editor_Api_Regular_Strings {
	 ...
	 *
	 * Hooked to wp_ajax_translentor_get_translations_regular
	 *       and wp_ajax_nopriv_translentor_get_translations_regular.
	 */
	public function get_translations() {
		
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			check_ajax_referer( 'get_translations', 'security' );
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'translentor_get_translations_regular' && !empty( $_POST['language'] ) && in_array( $_POST['language'], get_option('translentor_settings_elementor')['translentor_lang'] ) ) {
				...
				if ( is_array( $ids ) || is_array( $originals) ) {
					...
					$block_type = $this->lang_query->get_constant_block_type_regular_string();
					...
					$string_group = __('Others', 'translentor'); // this type is not registered in the string types because it will be overwritten by the content in data-translentor-node-type
					if ( isset( $_POST['dynamic_strings'] ) && $_POST['dynamic_strings'] === 'true'  ){
						$string_group = $localized_text['dynamicstrings'];
					}
					$dictionary_by_original = translentor_sort_dictionary_by_original( $dictionaries, 'regular', $string_group, sanitize_text_field( $_POST['language'] ) );

					echo translentor_safe_json_encode( $dictionary_by_original );//phpcs:ignore
				}
			}
		}
	...
	protected function get_translation_for_strings( $ids, $originals, $block_type = null, $skip_machine_translation = array() ){
		...
		if ( ! $this->translation_render ) {
			$this->translation_render = $translentor->get_translentorelements('translation_render');
		}
		if ( ! $this->lang_url ) {
		...
		foreach( $originals as $original ){
			if ( isset( $original ) ) {
				$trimmed_string = translentor_full_trim( translentor_sanitize_string( $original, false ) );
				if ( ( filter_var($trimmed_string, FILTER_VALIDATE_URL) === false) ){
					// not url
					$original_array[] = $trimmed_string;
				}else{
					// is url
					if ( $this->translation_render->is_external_link( $trimmed_string, $home_url ) || $this->lang_url->url_is_file( $trimmed_string ) ) {
						// allow only external url or file urls
						$original_array[] = remove_query_arg( 'translentor-editor', $trimmed_string );
					}
				}
			}
		}
		...
		if ( $this->settings['translentor-default-language'] != $current_language ) {
			if ( !empty ( $original_array ) && current_user_can ( apply_filters( 'translentor_translating_capability', 'manage_options' ) ) ) {
				$this->translation_render->process_strings($original_array, $current_language, $block_type, $skip_machine_translation);
			}
			$dictionaries[$current_language] = $this->lang_query->get_string_rows( $id_array, $original_array, $current_language );
		}else{
		...
		if ( isset( $_POST['all_languages'] ) && $_POST['all_languages'] === 'true' ) {
			foreach (get_option('translentor_settings_elementor')['translentor_lang'] as $language) {
				if ($language == $this->settings['translentor-default-language']) {
					$dictionaries[$language]['translentor-default-language'] = true;
					continue;
				}
				...
				$dictionaries[$language] = $this->lang_query->get_string_rows(array(), $original_strings, $language);
			}
		}
	 ...
	 *
	 * Hooked to wp_ajax_translentor_save_translations_regular.
	 */
	public function save_translations(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( apply_filters( 'translentor_translating_capability', 'manage_options' ) ) ) {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'translentor_save_translations_regular' && !empty( $_POST['strings'] ) ) {
		...
		echo translentor_safe_json_encode( $update_strings ); // phpcs:ignore
		die();
	}
	...
	protected function save_translations_of_strings( $strings, $block_type = null ){
		...
		foreach ( $strings as $language => $language_strings ) {
			if ( in_array( $language, get_option('translentor_settings_elementor')['translentor_lang'] ) && $language != $this->settings['translentor-default-language'] ) {
				...
				foreach( $language_strings as $string ) {
					if ( isset( $string->id ) && is_numeric( $string->id ) ) {
						...
						array_push($update_strings[ $language ], array(
							'id' => (int)$string->id,
							'original' => translentor_sanitize_string( $string->original, false ),
							'translated' => translentor_sanitize_string( $string->translated ),
		...
		foreach( $update_strings as $language => $update_string_array ) {
			$this->lang_query->update_strings( $update_string_array, $language, array('id','translated', 'status', 'block_type'));
			$this->lang_query->remove_possible_duplicates($update_string_array, $language, 'regular');
		}

        do_action('translentor_save_editor_translations_regular_strings', $update_strings, $this->settings);
	...
	public function create_translation_block(){
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( apply_filters( 'translentor_translating_capability', 'manage_options' ) ) ) {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'translentor_create_translation_block' && !empty( $_POST['strings'] ) && !empty( $_POST['language'] ) && in_array( $_POST['language'], get_option('translentor_settings_elementor')['translentor_lang'] ) && !empty( $_POST['original'] ) ) {
				$strings = json_decode( stripslashes( $_POST['strings'] ) ); /* phpcs:ignore */ /* sanitized downstream */

				if ( isset ( get_option('translentor_settings_elementor')['translentor_lang']) ){
					$translentor = translentor_Translentor::get_translentor_instance();
					if ( ! $this->lang_query ) {
						$this->lang_query = $translentor->get_translentorelements( 'query' );
					}
					...
					$active_block_type = $this->lang_query->get_constant_block_type_active();
					foreach( get_option('translentor_settings_elementor')['translentor_lang'] as $language ){
						if ( $language != $this->settings['translentor-default-language'] ){
					...
					if ( isset( $dictionaries ) ){
						foreach ( $dictionaries as $language => $dictionary ){
							...
							foreach( $dictionary as $dictionary_string_key => $dictionary_string ){
								...
								foreach( $ajax_translated_string_list as $ajax_key => $ajax_string ) {
									if ( translentor_full_trim( translentor_sanitize_string( $ajax_string->original, false ) ) == $dictionary_string->original ) {
										if ( $ajax_string->translated != '' ) {
											$dictionaries[ $language ][ $dictionary_string_key ]->translated = translentor_sanitize_string( $ajax_string->translated );
						...
						if ( $originals_array_constructed ){
							$table_names = $this->lang_query->get_all_table_names( $this->settings['translentor-default-language'], get_option('translentor_settings_elementor')['translentor_lang'] );
							if ( count( $table_names ) > 0 ){
								foreach( $table_names as $table_name ) {
									$originals = $copy_of_originals;
									$language = $this->lang_query->get_language_code_from_table_name( $table_name );
									$existing_dictionary = $this->lang_query->get_string_rows( array(), $originals, $language, ARRAY_A );
									...
									$this->lang_query->insert_strings( $originals, $language, $active_block_type );
									$this->lang_query->update_strings( $existing_dictionary, $language );
								}

							}
						}

						echo translentor_safe_json_encode( $dictionaries );//phpcs:ignore
					}
				}
	...
	public function split_translation_block() {
        if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( apply_filters( 'translentor_translating_capability', 'manage_options' ) ) ) {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] === 'translentor_split_translation_block' && ! empty( $_POST['strings'] ) ) {
                $raw_original_array = json_decode( stripslashes( $_POST['strings'] ) ); /* phpcs:ignore */ /* sanitized downstream */
				$translentor = translentor_Translentor::get_translentor_instance();
				if ( ! $this->lang_query ) {
					$this->lang_query = $translentor->get_translentorelements( 'query' );
				}
				$deprecated_block_type = $this->lang_query->get_constant_block_type_deprecated();
				$originals = array();
				foreach( $raw_original_array as $original ){
					$originals[] = translentor_sanitize_string( $original, false );
				}

				// even inactive languages ( not in get_option('translentor_settings_elementor')['translentor_lang'] array ) will be updated
				$all_languages_table_names = $this->lang_query->get_all_table_names( $this->settings['translentor-default-language'], array() );
				$rows_affected = $this->lang_query->update_translation_blocks_by_original( $all_languages_table_names, $originals, $deprecated_block_type );
				if ( $rows_affected == 0 ){
					// do updates individually if it fails
					foreach ( $all_languages_table_names as $table_name ){
						$this->lang_query->update_translation_blocks_by_original( array( $table_name ), $originals, $deprecated_block_type );
					}
				}
			}
        }
