 ...
 * Class Lang_Query
 *
 * Queries for translations in custom translentor tables.
 *
 */
class Lang_Query{
    ...
    public function get_existing_translations( $strings_array, $language_code, $block_type = null ){
        ...
        if( !$this->check_invalid_text ){
            ...
            $this->check_invalid_text = $translentor->get_translentorelements( 'check_invalid_text' );
        }
        ...
        $dictionary = apply_filters( 'translentor_get_existing_translations', $dictionary, $prepared_query, $strings_array, $language_code, $block_type );
        if ( is_array( $dictionary ) && count( $dictionary ) === 0 && !$this->table_exists($this->get_table_name( $language_code )) && !$this->check_invalid_text->is_invalid_data_error()){
            ...
            $this->maybe_record_automatic_translation_error(array( 'details' => 'Missing table ' . $this->get_table_name( $language_code ) . ' . To regenerate tables, try going to Settings->translentor->General tab and Save Settings.'), true );
        }
    ...
    public function check_machine_translation_log_table(){
        $table_name = $this->db->prefix . 'translentor_machine_translation_log';
        if ( $this->db->get_var( "SHOW TABLES LIKE '$table_name'" ) != $table_name )
        {
            // table not in database. Create new table
            $charset_collate = $this->db->get_charset_collate();
    ...
    public function copy_all_translation_blocks_into_table( $default_language, $language_code ){
    	...
    	if ( count( $all_table_names ) > 0 ){
		    ...
		    $source_language = apply_filters( 'translentor_source_language_translation_blocks', '', $default_language, $language_code );
		    if ( $source_language != '' ){
    ...
    public function original_ids_insert( $language_code, $inferior_limit, $batch_size ){
        ...
        if ( $this->settings['translentor-default-language'] === $language_code )
            return 0;

        if( !$this->error_manager ){
            $translentor = translentor_Translentor::get_translentor_instance();
            $this->error_manager = $translentor->get_translentorelements( 'error_manager' );
        }

        $originals_table = $this->get_table_name_for_original_strings();
        $table_name = sanitize_text_field( $this->get_table_name( $language_code, $this->settings['translentor-default-language'] ) );
    ...
    public function check_original_meta_table(){
        ...
        $table_name = $table_name = $this->db->get_blog_prefix() . 'translentor_original_meta';
        if ( $this->db->get_var( "SHOW TABLES LIKE '$table_name'" ) != $table_name ) {
            // table not in database. Create new table
            $charset_collate = $this->db->get_charset_collate();
    ...
    public function get_table_name( $language_code, $default_language = null ){
        ...
        return $this->db->prefix . 'translentor_dictionary_' . strtolower( $default_language ) . '_'. strtolower( $language_code );
    }
    ...
    public function get_language_code_from_table_name( $table_name, $default_language = null ){
	    ...
	    $language_code = str_replace($this->db->prefix . 'translentor_dictionary_' . strtolower( $default_language ) . '_', '', $table_name );
    ...
    public function get_table_name_for_original_strings(){
        return sanitize_text_field( $this->db->prefix . 'translentor_original_strings' );
    }
    ...
    public function get_table_name_for_original_meta(){
        return sanitize_text_field( $this->db->prefix . 'translentor_original_meta' );
    }
    /**
    ...
    public function get_all_gettext_strings(  $language_code ){
        ...
        if ( is_array( $dictionary ) && count( $dictionary ) === 0 && !$this->table_exists($this->get_gettext_table_name( $language_code )) ){
            ...
            $this->maybe_record_automatic_translation_error(array( 'details' => 'Missing table ' . $this->get_gettext_table_name( $language_code ). ' . To regenerate tables, try going to Settings->translentor->General tab and Save Settings.'), true );
        }
        return $dictionary;
    }
    ...
    public function get_gettext_table_name( $language_code ){
		global $wpdb;
        return $wpdb->get_blog_prefix() . 'translentor_gettext_' . strtolower( $language_code );
    }
    ...
    public function get_all_gettext_table_names(){
        global $wpdb;
        $table_name = $wpdb->get_blog_prefix() . 'translentor_gettext_';
        $return_tables = array();
	...
	public function update_translation_blocks_by_original( $table_names, $original_array, $block_type ) {
		...
		foreach( $table_names as $table_name ){
			...
			foreach( $original_array as $string ){
				$placeholders[] = '%s';
				$values[] = translentor_full_trim( $string );
			}
		}
	...
	public function maybe_record_automatic_translation_error($error_details = array(), $ignore_last_error = false ){
        ...
        if ( ( !empty( $this->db->last_error) && !$this->check_invalid_text->is_invalid_data_error() ) || $ignore_last_error ){
            ...
            $default_error_details = array(
                ...
                'url' => $this->lang_url->cur_page_url(),
            );
    ...
    public function rename_originals_table(){
        ...
        $table_to_use_for_recovery = get_option('translentor_original_strings_table_for_recovery', '');
        if ( $table_to_use_for_recovery == '' ) {
            ...
            update_option( 'translentor_original_strings_table_for_recovery', $new_table_name );
        }
    }
    ...
    public function regenerate_original_meta_table($inferior_limit, $batch_size){
        ...
        $originals_table = $this->get_table_name_for_original_strings();
        $recovery_originals_table = sanitize_text_field( get_option( 'translentor_original_strings_table_for_recovery' ) );
        $originals_meta_table = $this->get_table_name_for_original_meta();
        if ( empty( $recovery_originals_table ) ){
            $this->error_manager->record_error(array('regenerate_original_meta_table' => 'Empty option translentor_original_strings_table_for_recovery'));
            return;
        }

        $this->db->query( $this->db->prepare("UPDATE `$originals_meta_table` translentor_meta INNER JOIN `$recovery_originals_table` translentor_old ON translentor_meta.original_id = translentor_old.id LEFT JOIN `$originals_table` translentor_new ON translentor_new.original = translentor_old.original set translentor_meta.original_id = IF(translentor_new.id IS NULL, 0, translentor_new.id) WHERE translentor_meta.meta_id > %d AND translentor_meta.meta_id <= %d AND translentor_new.id != translentor_old.id", $inferior_limit, ($inferior_limit + $batch_size) ) );

        /* UPDATE `wp_translentor_original_meta` translentor_meta INNER JOIN `wp_translentor_original_strings1608214654` as translentor_old ON translentor_meta.original_id = translentor_old.id
         * LEFT JOIN `wp_translentor_original_strings` as translentor_new on translentor_new.original = translentor_old.original  set translentor_meta.original_id = IF(translentor_new.id IS NULL, 0, translentor_new.id)
         * WHERE translentor_meta.meta_id > 10 AND translentor_meta.meta_id <= 33 AND translentor_new.id != translentor_old.id*/
    ...
    public function is_values_accepted(){
        ...
        return apply_filters('translentor_is_sql_values_accepted', $return );
    }
