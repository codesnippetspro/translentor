...
/**
 * Class translentor_Translation_Render
 *
 * Translates pages.
 */
class translentor_Translation_Render{
    ...
    /**
     * translentor_Translation_Render constructor.
     *
    ...
    public function __construct( $settings ){
        ...
        $this->common_html_tags = implode( '|', apply_filters('translentor_common_html_tags', array( 'html', 'body', 'table', 'tbody', 'thead', 'th', 'td', 'tr', 'div', 'p', 'span', 'b', 'a', 'strong', 'center', 'br', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hr', 'img' ) ) );
    }
    ...
    public function start_output_buffer(){
        ...
        $ajax_on_frontend = translentor_Translation_Manager::is_ajax_on_frontend();//TODO refactor this function si it just checks and does not set variables

        if( ( is_admin() && !$ajax_on_frontend ) || translentor_is_translation_editor( 'true' ) ){
            ...
            global $translentor_output_buffer_started;//use this global so we know that we started the output buffer. we can check it for instance when wrapping gettext
            mb_http_output("UTF-8");
            if ( $translentor_LANGUAGE == $this->settings['translentor-default-language'] && !translentor_is_translation_editor() ) {
                // on default language when we are not in editor we just need to clear any translentor tags that could still be present and handle links for special situation
               // $chunk_size = ($this->handle_custom_links_for_default_language() ) ? null : 4096;
                //ob_start(array( $this, 'render_default_language' ), $chunk_size);
              
                $translentor_output_buffer_started = true;
            } else {
     ...
     */
    public function translentor_debug_mode_off(){
        if ( WP_DEBUG ) {
     ...
     * Otherwise we're just returning the global $translentor_LANGUAGE
     *
     * @return string       Language code.
     */
    protected function force_language_in_preview(){
        global $translentor_LANGUAGE;
        if ( in_array( $translentor_LANGUAGE, get_option('translentor_settings_elementor')['translentor_lang'] ) ) {
            if ( $translentor_LANGUAGE == $this->settings['translentor-default-language']  ){
                ...
                if ( isset( $_REQUEST['translentor-editor'] ) && $_REQUEST['translentor-editor'] == 'preview' )  {
                    if( count( get_option('translentor_settings_elementor')['translentor_lang'] ) > 1 ){
                        foreach (get_option('translentor_settings_elementor')['translentor_lang'] as $language) {
                            if ($language != $translentor_LANGUAGE) {
	...
	public function full_trim( $string ) {
		return translentor_full_trim( $string );
	}
    ...
    protected function get_node_type_category( $current_node_type ){
        ...
        $node_type_categories = apply_filters( 'translentor_node_type_categories', array(
    ...
    protected function get_node_description( $current_node ){
        $node_type_descriptions = apply_filters( 'translentor_node_type_descriptions',
            array(
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'name',
                    'value'         => 'description',
                    'description'   => esc_html__( 'Description', 'translentor' )
                ),
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'property',
                    'value'         => 'og:title',
                    'description'   => esc_html__( 'OG Title', 'translentor' )
                ),
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'property',
                    'value'         => 'og:site_name',
                    'description'   => esc_html__( 'OG Site Name', 'translentor' )
                ),
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'property',
                    'value'         => 'og:description',
                    'description'   => esc_html__( 'OG Description', 'translentor' )
                ),
	            array(
		            'type'          => 'meta_desc',
		            'attribute'     => 'property',
		            'value'         => 'og:image:alt',
		            'description'   => esc_html__( 'OG Image Alt', 'translentor' )
	            ),
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'name',
                    'value'         => 'twitter:title',
                    'description'   => esc_html__( 'Twitter Title', 'translentor' )
                ),
                array(
                    'type'          => 'meta_desc',
                    'attribute'     => 'name',
                    'value'         => 'twitter:description',
                    'description'   => esc_html__( 'Twitter Description', 'translentor' )
                ),
	            array(
		            'type'          => 'meta_desc',
		            'attribute'     => 'name',
		            'value'         => 'twitter:image:alt',
		            'description'   => esc_html__( 'Twitter Image Alt', 'translentor' )
	            ),
                array(
                    'type'          => 'page_title',
                    'description'   => esc_html__( 'Page Title', 'translentor' )
                ),
	            array(
		            'type'          => 'meta_desc',
		            'attribute'     => 'name',
		            'value'         => 'DC.Title',
		            'description'   => esc_html__( 'Dublin Core Title', 'translentor' )
	            ),
	            array(
		            'type'          => 'meta_desc',
		            'attribute'     => 'name',
		            'value'         => 'DC.Description',
		            'description'   => esc_html__( 'Dublin Core Description', 'translentor' )
	            ),
	            array(
		            'type'          => 'meta_desc_img',
		            'attribute'     => 'property',
		            'value'         => 'og:image',
		            'description'   => esc_html__( 'OG Image', 'translentor' )
	            ),
	            array(
		            'type'          => 'meta_desc_img',
		            'attribute'     => 'property',
		            'value'         => 'og:image:secure_url',
		            'description'   => esc_html__( 'OG Image Secure URL', 'translentor' )
	            ),
	            array(
		            'type'          => 'meta_desc_img',
		            'attribute'     => 'name',
		            'value'         => 'twitter:image',
		            'description'   => esc_html__( 'Twitter Image', 'translentor' )
	            ),
    ...
    public function trim_translation_block( $string ){
	    return preg_replace('/\s+/', ' ',   html_entity_decode( htmlspecialchars_decode( wp_strip_all_tags(translentor_full_trim( $string )), ENT_QUOTES ) ) ) ;
    }
    ...
    public function oembed_response_data($data, $post, $width, $height ){
        if ( !empty( $data )) {
            $translatable_items = apply_filters( 'translentor_oembed_response_data_translatable_items', array('title', 'html', 'provider_name') );
        ...
        remove_filter( 'translentor_before_translate_content', array( $this, 'handle_cdata'), 1000);
    ...
    public function translate_page( $output ){
    	if ( apply_filters( 'translentor_stop_translating_page', false, $output ) ){
    		return $output;
	    }

        global $translentor_HDOM_QUOTE_DEFAULT;
        $translentor_HDOM_QUOTE_DEFAULT = apply_filters('translentor_hdom_quote_default_double_quotes', '"');

    	global $translentor_editor_notices;

        /* replace our special tags so we have valid html */
        $output = str_ireplace('#!translentorst#', '<', $output);
        $output = str_ireplace('#!translentoren#', '>', $output);

        $output = apply_filters('translentor_before_translate_content', $output);
        ...
        if( is_object($wp_rewrite) ) {
            if( strpos( $this->lang_url->cur_page_url(), get_rest_url() ) !== false && strpos( current_filter(), 'rest_prepare_' ) !== 0 && current_filter() !== 'oembed_response_data' ){
                $translentorremoved = $this->remove_translentor_html_tags( $output );
                return $translentorremoved;
            }
        }

        /* don't do anything on xmlrpc.php  */
        if( strpos( $this->lang_url->cur_page_url(), 'xmlrpc.php' ) !== false ){
            $translentorremoved = $this->remove_translentor_html_tags( $output );
            return $translentorremoved;
        }
        ...
        if ( $language_code == $this->settings['translentor-default-language'] ){
        	// Don't translate regular strings (non-gettext) when we have no other translation languages except default language ( count( $this->settings['translentor_publish-lang'] ) > 1 )
        	$translate_normal_strings = false;
        }else{
	        $translate_normal_strings = true;
        }

	    $preview_mode = isset( $_REQUEST['translentor-editor'] ) && $_REQUEST['translentor-editor'] == 'preview';
	    ...
	    if( $json_array && $json_array != $output ) {
            ...
            if ( ! empty( $_REQUEST['action'] ) && strpos( sanitize_text_field( $_REQUEST['action'] ), 'translentor_' ) === 0 && $_REQUEST['action'] != 'translentor_split_translation_block' )
		        return $output;
	        ...
	        return translentor_safe_json_encode( $json_array );
        }
         ...
         */
        if( apply_filters( 'translentor_try_fixing_invalid_html', false ) ) {
	    ...
	    $html = translentor\str_get_html($output, true, true, translentor_DEFAULT_TARGET_CHARSET, false, translentor_DEFAULT_BR_TEXT, translentor_DEFAULT_SPAN_TEXT);
	    if ( $html === false ){
            $translentorremoved = $this->remove_translentor_html_tags( $output );
		    return $translentorremoved;
	    }
	    ...
	    if ( $translate_normal_strings ) {
		    $all_existing_translation_blocks = $this->lang_query->get_all_translation_blocks( $language_code );
        ...
        /**
         * When we are in the translation editor: Intercept the translentor-gettext that was wrapped around all the gettext texts, grab the attribute data-translentorgettextoriginal
         * which contains the original translation id and move it to the parent node if the parent node only contains that string then remove the  wrap translentor-gettext, otherwise replace it with another tag.
         * Also set a no-translation attribute.
         * When we are in a live translation case: Intercept the translentor-gettext that was wrapped around all the gettext texts, set a no-translation attribute to the parent node if the parent node only contains that string
         * then remove the  wrap translentor-gettext, otherwise replace the wrap with another tag and do the same to it
         * We identified two cases: the wrapper translentor-gettext can be as a node in the dome or ot can be inside a html attribute ( for example value )
         * and we need to treat them differently
         */

        /* store the nodes in arrays so we can sort the $translentor_rows which contain translentor-gettext nodes from the DOM according to the number of children and we process the simplest first */
        $translentor_rows = array();
        $translentor_attr_rows = array();
        foreach ( $html->find("*[!nuartrebuisaexiteatributulasta]") as $k => $row ){
            if( $row->hasAttribute('data-translentorgettextoriginal') ){
                $translentor_rows[count( $row->children )][] = $row;
            }
            else{
                if( $row->nodetype !== 5 && $row->nodetype !== 3 )//add all tags that are not root or text, text nodes can't have attributes
                    $translentor_attr_rows[] = $row;
	            ...
	            if ( $translate_normal_strings && $count_translation_blocks > 0 ) {
		            ...
		            if ( $translation_block ) {
			            ...
			            if ( $translation_block->block_type == 1 ) {
				            ...
				            $row->setAttribute( 'data-translentor-translate-id', $translation_block->id );
				            $row->setAttribute( 'data-translentor-translate-id-deprecated', $translation_block->id );
				            $row->setAttribute( 'class', $existing_classes . 'translentor-deprecated-tb' );
			            }
		            }
	            }
        ...
        ksort($translentor_rows);
        foreach( $translentor_rows as $level ){
            foreach( $level as $row ){
                $original_gettext_translation_id = $row->getAttribute('data-translentorgettextoriginal');
                ...
                if( count( $row->parent()->children ) == 1 && $row->parent()->innertext == $row->outertext ){
	                ...
	                $row->parent()->setAttribute('data-translentor-gettext', '');
                    // we are in the editor
                    if (isset($_REQUEST['translentor-editor']) && $_REQUEST['translentor-editor'] == 'preview') {
                        //move up the data-translentorgettextoriginal attribute
                        $row->parent()->setAttribute('data-translentorgettextoriginal', $original_gettext_translation_id);
                    }
                }
                else{
                    ...
                    $row->outertext = '<translentor-wrap class="translentor-wrap" data-no-translation';
                    if (isset($_REQUEST['translentor-editor']) && $_REQUEST['translentor-editor'] == 'preview') {
                        $row->outertext .= ' data-translentorgettextoriginal="'. $original_gettext_translation_id .'"';
                    }
                    $row->outertext .= '>'.$row->innertext().'</translentor-wrap>';
                }
            }
        }

        foreach( $translentor_attr_rows as $row ){
            $all_attributes = $row->getAllAttributes();
            if( !empty( $all_attributes ) ) {
                foreach ($all_attributes as $attr_name => $attr_value) {
                    if (strpos($attr_value, 'translentor-gettext ') !== false) {
                        //if we have json content in the value of the attribute, we don't do anything. The translentor-wrap will be removed later in the code
                        if (is_array($json_array = json_decode( html_entity_decode( $attr_value, ENT_QUOTES ), true ) ) ) {
                            continue;
                        }

                        // convert to a node
                        $node_from_value = translentor\str_get_html(html_entity_decode(htmlspecialchars_decode($attr_value, ENT_QUOTES)), true, true, translentor_DEFAULT_TARGET_CHARSET, false, translentor_DEFAULT_BR_TEXT, translentor_DEFAULT_SPAN_TEXT);
	                    if ( $node_from_value === false ){
		                    continue;
	                    }
                        foreach ($node_from_value->find('translentor-gettext') as $nfv_row) {
                            ...
                            // we are in the editor
                            if (isset($_REQUEST['translentor-editor']) && $_REQUEST['translentor-editor'] == 'preview') {
                                $original_gettext_translation_id = $nfv_row->getAttribute('data-translentorgettextoriginal');
                                $row->setAttribute('data-translentorgettextoriginal-' . $attr_name, $original_gettext_translation_id);
                            }
	    ...
	    if ( ! $translate_normal_strings ) {
            /* save it as a string */
            $translentorremoved = $html->save();
            /* perform preg replace on the remaining translentor-gettext tags */
            $translentorremoved = $this->remove_translentor_html_tags($translentorremoved );
		    return $translentorremoved;
	    }

        $no_translate_selectors = apply_filters( 'translentor_no_translate_selectors', array( '#wpadminbar' ), $translentor_LANGUAGE );
        $ignore_cdata = apply_filters('translentor_ignore_cdata', true );
        $translate_encoded_html_as_string = apply_filters('translentor_translate_encoded_html_as_string', false );
        $translate_encoded_html_as_html = apply_filters('translentor_translate_encoded_html_as_html', true );
        // used for skipping minified scripts but can be used for anything
        $skip_strings_containing_key_terms = apply_filters('translentor_skip_strings_containing_key_terms',
            array(
                array(
        ...
        $no_auto_translate_selectors = apply_filters( 'translentor_no_auto_translate_selectors', array( ), $translentor_LANGUAGE );
        ...
        foreach ( $html->find('.translation-block') as $row ){
            $trimmed_string = translentor_full_trim( $row->innertext );
            $parent = $row->parent();
            if( $trimmed_string!=""
                && $parent->tag!="script"
                && $parent->tag!="style"
                ...
                && !$this->translentor_is_numeric($trimmed_string)
                ...
                && !$this->has_ancestor_attribute( $row, $no_translate_attribute ) )
            {
                ...
                //add data-translentor-post-id attribute if needed
                $nodes = $this->maybe_add_post_id_in_node( $nodes, $row, $string_count );
            }
        }

        foreach ( $html->find('translentortext') as $row ){
            $outertext = $row->outertext;
            $parent = $row->parent();
            $trimmed_string = translentor_full_trim( $outertext );
            if( $trimmed_string!=""
                && $parent->tag!="script"
                && $parent->tag!="style"
                && $parent->tag != 'title'
                ...
                && !$this->contains_substrings($trimmed_string, $skip_strings_containing_key_terms ) )
            {
                if ( !$translate_encoded_html_as_string ){
                    ...
                    if ( $translate_encoded_html_as_html ){
                        if ( $this->is_html($trimmed_string) ){
                            ...
                            add_filter( 'translentor_translate_encoded_html_as_html', '__return_false' );

                            $row->outertext = str_replace( $trimmed_string, $this->translate_page( $trimmed_string ), $row->outertext );
                            remove_filter( 'translentor_translate_encoded_html_as_html', '__return_false' );
                            $is_html = true;
                        }else {
                ...
                if ( ! apply_filters( 'translentor_allow_machine_translation_for_string', true, $trimmed_string, null, null, $row ) ){
            ...
            $row = apply_filters( 'translentor_process_other_text_nodes', $row );
	    ...
	    foreach( $node_accessors as $node_accessor_key => $node_accessor ){
	    	if ( isset( $node_accessor['selector'] ) ){
			    foreach ( $html->find( $node_accessor['selector'] ) as $k => $row ){
				    ...
				    $trimmed_string = translentor_full_trim( $row->$current_node_accessor_selector );
			    	if ( $current_node_accessor_selector === 'href' ) {
					    $translate_href = ( $this->is_external_link( $trimmed_string, $home_url ) || $this->lang_url->url_is_file( $trimmed_string ) || $this->lang_url->url_is_extra($trimmed_string) );
					    $translate_href = apply_filters( 'translentor_translate_this_href', $translate_href, $row, $translentor_LANGUAGE );
                        ...
                        && !$this->contains_substrings($trimmed_string, $skip_strings_containing_key_terms ) )
				    {
					    ...
					    if ( ! apply_filters( 'translentor_allow_machine_translation_for_string', true, $entity_decoded_trimmed_string, $current_node_accessor_selector, $node_accessor, $row ) ){
        ...
        $translateable_information = apply_filters( 'translentor_translateable_strings', $translateable_information, $html, $no_translate_attribute, $translentor_LANGUAGE, $language_code, $this );
        ...
        do_action('translentor_translateable_information', $translateable_information, $translated_strings, $language_code);
        ...
        if( !empty($translateable_information['nodes']) ){
            ...
            if( !empty( $strings_in_post_content ) ){
                ...
                if( $set_meta_for_this_url === false ){
                    ...
                    $original_string_ids = $this->lang_query->get_original_string_ids($strings_in_post_content['strings']);

                    if( !empty( $original_string_ids ) ){
                        //there is a correlation between the two arrays
                        $this->lang_query->set_original_string_meta_post_id( $original_string_ids, $strings_in_post_content['post_ids'] );
                    }
        ...
        if ( $preview_mode ) {
            $translated_string_ids = $this->lang_query->get_string_ids($translateable_strings, $language_code);
        }
        ...
        foreach ( $nodes as $i => $node ) {
            ...
            if ( $translation_available && isset( $current_node_accessor ) && ! ( $preview_mode && ( $this->settings['translentor-default-language'] == $translentor_LANGUAGE ) ) ) {
                ...
                if ( $current_node_accessor[ 'attribute' ] ){
                    ...
                    do_action( 'translentor_set_translation_for_attribute', $nodes[$i]['node'], $accessor, $translated_strings[$i] );
                }else{
	                $translateable_string = $this->maybe_correct_translatable_string( $translateable_string, $nodes[$i]['node']->$accessor );
                    $nodes[$i]['node']->$accessor = str_replace( $translateable_string, translentor_sanitize_string($translated_strings[$i]), $nodes[$i]['node']->$accessor );
                }
            ...
            if ( $preview_mode ) {
                if ( $accessor == 'outertext' && $nodes[$i]['type'] != 'button' ) {
                    $outertext_details = '<translentor-element data-translentor-translate-id="' . $translated_string_ids[$translateable_strings[$i]]->id . '" data-translentor-node-group="' . $this->get_node_type_category( $nodes[$i]['type'] ) . '"';
                    if ( $this->get_node_description( $nodes[$i] ) ) {
                        $outertext_details .= ' data-translentor-node-description="' . $this->get_node_description($nodes[$i] ) . '"';
                    }
                    $outertext_details .= '>' . $nodes[$i]['node']->outertext . '</translentor-element>';
                    $nodes[$i]['node']->outertext = $outertext_details;
                } else {
                    if( $nodes[$i]['type'] == 'button' || $nodes[$i]['type'] == 'option' ){
                        $nodes[$i]['node'] = $nodes[$i]['node']->parent();
                    }
	                $nodes[$i]['node']->setAttribute('data-translentor-translate-id-' . $accessor, $translated_string_ids[ $translateable_strings[$i] ]->id );
                    $nodes[$i]['node']->setAttribute('data-translentor-node-group-' . $accessor, $this->get_node_type_category( $nodes[$i]['type'] ) );

                    if ( $this->get_node_description( $nodes[$i] ) ) {
                        $nodes[$i]['node']->setAttribute('data-translentor-node-description-' . $accessor, $this->get_node_description($nodes[$i]));
                    }
        ...
        if( apply_filters('tp_handle_custom_links_in_translation_blocks', false) ) {
            ...
            $html = translentor\str_get_html($html_string, true, true, translentor_DEFAULT_TARGET_CHARSET, false, translentor_DEFAULT_BR_TEXT, translentor_DEFAULT_SPAN_TEXT);
            if ( $html === false ){
                return $html_string;
            }
        }
	    ...
	    $translentor_editor_notices = apply_filters( 'translentor_editor_notices', $translentor_editor_notices );
	    if ( translentor_is_translation_editor('preview') && $translentor_editor_notices != '' ){
		    $body = $html->find('body', 0 );
		    $body->innertext = '<div data-no-translation class="translentor-editor-notices">' . $translentor_editor_notices . "</div>" . $body->innertext;
	    }
	    $final_html = $html->save();

        /* perform preg replace on the remaining translentor-gettext tags */
        $final_html = $this->remove_translentor_html_tags( $final_html );

	    return apply_filters( 'translentor_translated_html', $final_html, $translentor_LANGUAGE, $language_code, $preview_mode );
    }
    ...
    public function handle_custom_links_and_forms( $html ){
        ...
        foreach( $html->find('a[href!="#"]') as $a_href)  {
            $a_href->href = apply_filters( 'translentor_href_from_translated_page', $a_href->href, $this->settings['translentor-default-language'] );
            ...
            if( $preview_mode && ! $is_external_link ){
                $a_href->setAttribute( 'data-translentor-original-href', $url );
            }

            if (
            	( $translentor_LANGUAGE != $this->settings['translentor-default-language'] || $this->settings['translentor-subdirectory-default-lang'] == 'yes' ) &&
                 
	            !$is_external_link &&
                !$this->lang_url->url_is_file( $url ) &&
                ( $this->lang_url->get_lang_from_url_string( $url ) == null || ( isset ($this->settings['translentor-subdirectory-default-lang']) && $this->settings['translentor-subdirectory-default-lang'] === 'yes' && $this->lang_url->get_lang_from_url_string( $url ) === $this->settings['translentor-default-language'] ) ) &&
	            !$is_admin_link &&
                strpos($url, '#translentorLINKPROCESSED') === false &&
	            ( !$this->has_ancestor_attribute( $a_href, $no_translate_attribute ) || $this->has_ancestor_attribute($a_href, 'data-translentor-gettext') ) // add language param to link if it's inside a gettext
            ){
                $a_href->href = apply_filters( 'translentor_force_custom_links', $this->lang_url->get_url_for_language( $translentor_LANGUAGE, $url ), $url, $translentor_LANGUAGE, $a_href );
                $url = $a_href->href;
            }

            if( $preview_mode && ( $is_external_link || $this->is_different_language( $url ) || $is_admin_link ) ) {
                $a_href->setAttribute( 'data-translentor-unpreviewable', 'translentor-unpreviewable' );
            }

            $a_href->href = str_replace('#translentorLINKPROCESSED', '', $a_href->href);
        }
        ...
        foreach ( $html->find('form') as $k => $row ){
            ...
            if(!$is_admin_link) {
                $row->setAttribute( 'data-translentor-original-action', $row->action );
                $row->innertext .= apply_filters( 'translentor_form_inputs', '<input type="hidden" name="translentor-form-language" value="' . $this->settings['translentor-lang-slugs'][ $translentor_LANGUAGE ] . '"/>', $translentor_LANGUAGE, $this->settings['translentor-lang-slugs'][ $translentor_LANGUAGE ], $row );
                ...
                if ( !empty( $form_action )
                    ...
                    && !$is_external_link
                    && strpos( $form_action, '#translentorLINKPROCESSED' ) === false ) {
                    $row->action = $this->lang_url->get_url_for_language( $translentor_LANGUAGE, $form_action );
                }
                $row->action = str_replace( '#translentorLINKPROCESSED', '', $row->action );
            }
        }

        foreach ( $html->find('link') as $link ){
            $link->href = str_replace('#translentorLINKPROCESSED', '', $link->href);
        }
        return $html;
    }

    public function is_first_language_not_default_language(){
        return ( isset( $this->settings['translentor-subdirectory-default-lang'] ) &&
            $this->settings['translentor-subdirectory-default-lang'] == 'yes' &&
            isset( $this->settings['translentor_publish-lang'][0] ) &&
            $this->settings['translentor-default-language'] != $this->settings['translentor_publish-lang'][0] );
    }
    ...
    public function maybe_add_post_id_in_node( $nodes, $row, $string_count ){
        $post_container_node = $this->has_ancestor_attribute( $row, 'data-translentor-post-id' );
        if( $post_container_node && $post_container_node->attr['data-translentor-post-id'] ) {
            $nodes[$string_count - 1]['post_id'] = $post_container_node->attr['data-translentor-post-id'];
        }
        return $nodes;
    }
     ...
     *
     * Hooked to translentor_set_translation_for_attribute
     */
     ...
     *
     * Hooked to translentor_allow_machine_translation_for_string
     */
    public function allow_machine_translation_for_string( $allow, $entity_decoded_trimmed_string, $current_node_accessor_selector, $node_accessor ){
    	$skip_attributes = apply_filters( 'translentor_skip_machine_translation_for_attr', array( 'href', 'src' ) );
    ...
    /**
     * function that removes any unwanted leftover <translentor-gettext> tags
     * @param $string
     * @return string|string[]|null
     */
    function remove_translentor_html_tags( $string ){
        $string = preg_replace( '/(<|&lt;)translentor-gettext (.*?)(>|&gt;)/i', '', $string );
        $string = preg_replace( '/(<|&lt;)(\\\\)*\/translentor-gettext(>|&gt;)/i', '', $string );

        // In case we have a gettext string which was run through rawurlencode(). See more details on iss6563
        $string = preg_replace( '/%23%21translentorst%23translentor-gettext(.*?)%23%21translentoren%23/i', '', $string );
        $string = preg_replace( '/%23%21translentorst%23%2Ftranslentor-gettext%23%21translentoren%23/i', '', $string );

        if (!isset($_REQUEST['translentor-editor']) || $_REQUEST['translentor-editor'] != 'preview') {
            $string = preg_replace('/(<|&lt;)translentor-wrap (.*?)(>|&gt;)/i', '', $string);
            $string = preg_replace('/(<|&lt;)(\\\\)*\/translentor-wrap(>|&gt;)/i', '', $string);
        }

        //remove post containers before outputting
        $string = preg_replace( '/(<|&lt;)translentor-post-container (.*?)(>|&gt;)/i', '', $string );
        $string = preg_replace( '/(<|&lt;)(\\\\)*\/translentor-post-container(>|&gt;)/i', '', $string );
    ...
    function translate_json (&$value) {
        ...
        if ( $html_decoded_value != strip_tags( $html_decoded_value ) ) {
            ...
            /*the translentor-element tag can appear on a gettext string without html and should not be left in the json
            as we don't know how it will be inserted into the page by js */
            $value = preg_replace( '/(<|&lt;)translentor-element (.*?)(>|&gt;)/', '', $value );
            $value = preg_replace( '/(<|&lt;)(\\\\)*\/translentor-element(>|&gt;)/', '', $value );
        }
    }
	...
	function custom_links_and_forms_json (&$value) {
		...
		$html_decoded_value = html_entity_decode( (string) $value );
		if ( $html_decoded_value != strip_tags( $html_decoded_value ) ) {
			$html = translentor\str_get_html( $value, true, true, translentor_DEFAULT_TARGET_CHARSET, false, translentor_DEFAULT_BR_TEXT, translentor_DEFAULT_SPAN_TEXT );
			if( $html ) {
                $html = $this->handle_custom_links_and_forms($html);
                $value = $html->save();
            }
     ...
     * any translentor gettext tags that we added and for some reason show up  although they should not
     * @param $output
     * @return mixed
     */
    public function render_default_language( $output ){
	        ...
	        if( $json_array && $json_array != $output ) {
		        ...
		        if ( ! empty( $json_array ) ) {
			        if( is_array( $json_array ) ) {
				        ...
				        $html = translentor\str_get_html( $json_array, true, true, translentor_DEFAULT_TARGET_CHARSET, false, translentor_DEFAULT_BR_TEXT, translentor_DEFAULT_SPAN_TEXT );
				        if( $html ) {
                            $html = $this->handle_custom_links_and_forms($html);
        ...
        return translentor_Translation_Manager::strip_gettext_tags($output);
    }
    ...
    protected function is_different_language( $url ){
        ...
        $lang = $this->lang_url->get_lang_from_url_string( $url );
        if ( $lang == null ){
            $lang = $this->settings['translentor-default-language'];
        }
        if ( $lang == $translentor_LANGUAGE ){
            return false;
        }else{
            return true;
        }
    ...
    protected function is_admin_link( $url, $admin_url = '', $wp_login_url = '' ){
	    ...
	    return apply_filters('translentor_is_admin_link', $is_admin_link, $url, $admin_url, $wp_login_url);
    ...
    public function process_strings( $translateable_strings, $language_code, $block_type = null, $skip_machine_translating_strings = array() ){
        ...
        {
	    $machine_translation_available = $this->machine_translator->is_available( array( get_option('translentor_settings_elementor')['translentor-default-language']));
        }else{
            $machine_translation_available=false;
        }
        ...
        $dictionary = $this->lang_query->get_existing_translations( array_values($translateable_strings), $language_code );
        if ( $dictionary === false ){
        	return array();
        }
        $new_strings = array();
        ...
        foreach( $translateable_strings as $i => $string ){
	        ...
	        if ( isset( $dictionary[$string]->translated ) && $dictionary[$string]->status == $this->lang_query->get_constant_machine_translated() && $skip_string ){
	        	continue;
	        }
        ...
        $untranslated_list = $this->lang_query->get_untranslated_strings( $new_strings, $language_code );
        $update_strings = array();
       
        if ( $machine_translation_available ) {
           
            $machine_strings = $this->machine_translator->translate( $machine_translatable_strings, $language_code, get_option('translentor_settings_elementor')['translentor-default-language'] );
            $unique_original_strings_with_machine_translations = array_keys($machine_strings);
            $original_inserts = $this->lang_query->original_strings_sync( $language_code, $unique_original_strings_with_machine_translations );
            ...
            foreach ( $unique_original_strings_with_machine_translations as $string ) {
                ...
                array_push( $update_strings, array(
                    ...
                    'original'    => translentor_sanitize_string( $string, false ),
                    'translated'  => translentor_sanitize_string( $machine_strings[ $string ] ),
        ...
        $this->lang_query->insert_strings( $new_strings, $language_code, $block_type );
        $this->lang_query->update_strings( $update_strings, $language_code, array( 'id','original', 'translated', 'status', 'original_id' ) );
    ...
    public function get_node_accessors(){
	    return apply_filters( 'translentor_node_accessors', array(
		    'text' => array(
			    'accessor' => 'outertext',
			    'attribute' => false
		    ),
	...
	public function enqueue_scripts() {
		...
		if ( class_exists( 'WooCommerce' ) ){
			wp_enqueue_script('translentor-frontend-compatibility', translentor_URL . 'translentor/assets/js/translentor-frontend-compatibility.min.js', array(), translentor_VERSION );
		}

	}

	public function get_translentor_data(){
		global $translentor_LANGUAGE;
		...
		$language_to_query = $translentor_LANGUAGE;
		if ( $translentor_LANGUAGE == $this->settings['translentor-default-language']  ) {
			foreach (get_option('translentor_settings_elementor')['translentor_lang'] as $language) {
				if ( $language != $this->settings['translentor-default-language'] ) {
					$language_to_query = $language;
					break;
				}
			}
		}
		$language_to_query = ( count ( get_option('translentor_settings_elementor')['translentor_lang'] ) < 2 ) ? '' : $language_to_query;

		return array(
            'translentor_custom_ajax_url'                                  => apply_filters( 'translentor_custom_ajax_url', translentor_URL . 'translentor/classes/translentor-ajax.php' ),
            'translentor_wp_ajax_url'                                      => apply_filters( 'translentor_wp_ajax_url', admin_url( 'admin-ajax.php' ) ),
            'translentor_language_to_query'                                => $language_to_query,
            'translentor_original_language'                                => $this->settings['translentor-default-language'],
            'translentor_current_language'                                 => $translentor_LANGUAGE,
            'translentor_skip_selectors'                                   => apply_filters( 'translentor_skip_selectors_from_dynamic_translation', array( '[data-no-translation]', '[data-no-dynamic-translation]', '[data-translentor-translate-id-innertext]', 'script', 'style', 'head', 'translentor-span', 'translentor-element' ), $translentor_LANGUAGE, $this->settings ), // data-translentor-translate-id-innertext refers to translation block and it shouldn't be detected
            'translentor_base_selectors'                                   => $this->get_base_attribute_selectors(),
            'translentor_attributes_selectors'                             => $this->get_node_accessors(),
            'translentor_attributes_accessors'                             => $this->get_accessors_array(),
            'gettranslationsnonceregular'                          => $nonces['gettranslationsnonceregular'],
            'showdynamiccontentbeforetranslation'                  => apply_filters( 'translentor_show_dynamic_content_before_translation', false ),
            'skip_strings_from_dynamic_translation'                => apply_filters( 'translentor_skip_strings_from_dynamic_translation', array() ),
            'skip_strings_from_dynamic_translation_for_substrings' => apply_filters( 'translentor_skip_strings_from_dynamic_translation_for_substrings', array( 'href' => array('amazon-adsystem', 'googleads', 'g.doubleclick') ) ),
            'duplicate_detections_allowed'                         => apply_filters( 'translentor_duplicate_detections_allowed', 100 ),
            'translentor_translate_numerals_opt'                           => isset ($this->settings["translentor_advanced_settings"]["enable_numerals_translation"]) ? $this->settings["translentor_advanced_settings"]["enable_numerals_translation"] : 'no',
            'translentor_no_auto_translation_selectors'                    => apply_filters( 'translentor_no_auto_translate_selectors', array( '[data-no-auto-translation]' ), $translentor_LANGUAGE )
		);
	}

    /**
     * Enqueue translation script.
     */
    public function enqueue_dynamic_translation(){
        $enable_dynamic_translation = apply_filters( 'translentor_enable_dynamic_translation', true );
        ...
        if ( $translentor_LANGUAGE != $this->settings['translentor-default-language'] || ( isset( $_REQUEST['translentor-editor'] ) && $_REQUEST['translentor-editor'] == 'preview' ) ) {

            wp_enqueue_script('translentor-dynamic-translator', translentor_URL . 'translentor/assets/js/translentor-translate-dom-changes.js', array('jquery'), translentor_VERSION, true );
            wp_localize_script('translentor-dynamic-translator', 'translentor_data', $this->get_translentor_data() );
        }
    }

	/**
	 * Skip base selectors (data-translentor-translate-id, data-translentorgettextoriginal etc.)
	 *
	 * The base selectors (without any suffixes) are placed only if their children do not contain any nodes that are translatable
	 *
 	 * hooked to translentor_skip_selectors_from_dynamic_translation
	 *
	...
	public function get_base_attribute_selectors(){
		return apply_filters( 'translentor_base_attribute_selectors', array( 'data-translentor-translate-id', 'data-translentorgettextoriginal', 'data-translentor-post-slug' ) );
	}
     ...
     * @since 1.0.8
     */
    public function translentor_force_preview_on_url_redirect( $location, $status ){
        if( isset( $_REQUEST['translentor-editor'] ) && $_REQUEST['translentor-editor'] == 'preview' ){
            $location = add_query_arg( 'translentor-editor', 'preview', $location );
        }
        return $location;
    }

    /**
     * Filters the location redirect to add the current language based on the translentor-form-language parameter
     * @param $location
     * @param $status
     * @return string
     * @since 1.1.2
     */
    public function translentor_force_language_on_form_url_redirect( $location, $status ){
        if( isset( $_REQUEST[ 'translentor-form-language' ] ) && !empty($_REQUEST[ 'translentor-form-language' ]) ){
            $form_language_slug = sanitize_text_field($_REQUEST[ 'translentor-form-language' ]);
            $form_language = array_search($form_language_slug, $this->settings['translentor-lang-slugs']);
            if ( ! $this->lang_url ) {
                $translentor = translentor_Translentor::get_translentor_instance();
                $this->lang_url = $translentor->get_translentorelements('lang_url');
            }

            $location = $this->lang_url->get_url_for_language( $form_language, $location );
        }
        return $location;
    }
    ...
    public function force_preview_on_url_in_ajax( $output ){
        if ( translentor_Translation_Manager::is_ajax_on_frontend() && isset( $_REQUEST['translentor-editor'] ) && $_REQUEST['translentor-editor'] === 'preview' && $output != false ) {
            ...
            if ( json_last_error() === JSON_ERROR_NONE) {
                ...
                $output = translentor_safe_json_encode($result);
            } //endif
        } //endif
        return $output;
    }
    ...
    function callback_add_preview_arg(&$item, $key){
        if ( filter_var($item, FILTER_VALIDATE_URL) !== FALSE ) {
            $item = add_query_arg( 'translentor-editor', 'preview', $item );
        }
    }
    ...
    public function force_form_language_on_url_in_ajax( $output ){
        if ( translentor_Translation_Manager::is_ajax_on_frontend() && isset( $_REQUEST[ 'translentor-form-language' ] ) && !empty( $_REQUEST[ 'translentor-form-language' ] ) ) {
            $result = json_decode($output, TRUE);
            if ( json_last_error() === JSON_ERROR_NONE) {
    ...
    function callback_add_language_to_url(&$item, $key){
        if ( filter_var($item, FILTER_VALIDATE_URL) !== FALSE ) {
            $form_language_slug = isset( $_REQUEST[ 'translentor-form-language' ] ) ? sanitize_text_field($_REQUEST[ 'translentor-form-language' ]) : '';
            $form_language = array_search($form_language_slug, $this->settings['translentor-lang-slugs']);
            if ( ! $this->lang_url ) {
                $translentor = translentor_Translentor::get_translentor_instance();
                $this->lang_url = $translentor->get_translentorelements('lang_url');
            ...
            $item = $this->lang_url->get_url_for_language( $form_language, $item );
        }
    }
    ...
    function fix_wptexturize_characters( $translated, $text, $context, $domain ){
        ...
        if( $default_language != $translentor_LANGUAGE && array_key_exists($context, $list_of_context_text) && in_array($text, $list_of_context_text) ){
            return translentor_x( $text, $context, '', $default_language );
        }
    ...
    /**
     * Function that wraps the post title and the post content in a custom translentor wrap translentor-post-container so we can know in
    ...
    function wrap_with_post_id( $content, $id = null ){
        global $post, $translentor_LANGUAGE, $wp_query;

        if( empty($post->ID) )
            return $content;

        //we try to wrap only the actual content of the post and not when the filters are executed in SEO plugins for example
        if( ( !$wp_query->in_the_loop || !is_main_query() ) && apply_filters('translentor_wrap_with_post_id_overrule', true ) )
            return $content;
        ...
        if ( $translentor_LANGUAGE !== $this->settings['translentor-default-language'] ) {
            if ( is_singular() && !empty($post->ID)) {
                $content = "<translentor-post-container data-translentor-post-id='" . $post->ID . "'>" . $content . "</translentor-post-container>";//changed " to ' to not break cases when the filter is applied inside an html attribute (title for example)
            }
        }
	 ...
	 * @return bool
	 */
    function translentor_is_numeric($str){
    	if (is_numeric($str)){
    		if (isset($this->settings["translentor_advanced_settings"]["enable_numerals_translation"]) && $this->settings["translentor_advanced_settings"]["enable_numerals_translation"] === 'yes') {
	            return false;
		        } else {
			    return true;
		        }
