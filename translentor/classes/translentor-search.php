...
/**
 * Class translentor_Search
 *
 * Queries for translations in custom translentor tables.
 *
 */
class translentor_Search extends WP_Query{
    ...
    /**
     * translentor_Search constructor.
     * @param $settings
     */
     ...
     * @return mixed
     */
    public function translentor_search_filter( $query ) {
        global $translentor_LANGUAGE;

        if ( $translentor_LANGUAGE !== $this->settings['translentor-default-language'] ) {
            if ( ( !is_admin() && $query->is_main_query() && $query->is_search() ) || apply_filters( 'translentor_force_search', false ) ) {
    ...
    public function get_post_ids_containing_search_term($search_query, $query = null ){
        ...
        $translentor_search_query = '';
        $dictionary_name = $this->lang_query->get_table_name( apply_filters( 'translentor_change_search_dictionary_language', $translentor_LANGUAGE, $this, $query ) );
        $meta_table_name =  $this->lang_query->get_table_name_for_original_meta();

        if( $search_terms_count === 1 ){
            /**
             * for one search term we can find it directly in the translated column
             */
            $translentor_search_query = $this->db->prepare( "SELECT meta_value FROM
                                        $dictionary_name
                                        INNER JOIN $meta_table_name ON $dictionary_name.original_id = $meta_table_name.original_id AND $meta_table_name.meta_key = '". $this->lang_query->get_meta_key_for_post_parent_id() ."'
             ...
             */
            $translentor_search_query = "SELECT meta_value FROM 
        ...
        $search_result_ids = $this->db->get_results( $translentor_search_query, OBJECT_K );
     ...
     * @return string
     */
    public function translentor_search_query( $s ){
        global $translentor_LANGUAGE;
