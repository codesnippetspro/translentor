...
class translentor_Lang_Url {
    ...
    /**
     * translentor_Lang_Url constructor.
     *
    ...
    public function add_language_to_home_url( $url, $path, $orig_scheme, $blog_id ){
        ...
        //if this is not set then don't do anything as this is an exception/error and $translentor_LANGUAGE should always be set
        if( empty( $translentor_LANGUAGE ) )
            return $url;

        if ( isset( $this->settings['translentor-subdirectory-default-lang'] ) && $this->settings['translentor-subdirectory-default-lang'] == 'no' && $translentor_LANGUAGE == $this->settings['translentor-default-language'] ) {
            return $url;
        }

        if( apply_filters( 'translentor_add_language_to_home_url_check_for_admin', true, $url, $path ) &&
            ( is_customize_preview() || $this->is_admin_request()  || $this->is_sitemap_path( $path ) || $this->url_is_file( $path ) ) )
            return $url;

        $url_slug = $this->get_url_slug( $translentor_LANGUAGE );
        ...
        if ( translentor_force_slash_at_end_of_link( $this->settings ) ) {
        ...
        return apply_filters( 'translentor_home_url', $new_url, $abs_home, $translentor_LANGUAGE, $path, $url );
    }
    ...
    public function add_hreflang_to_head() {
        ...
        $original_url     = str_replace( '#translentorLINKPROCESSED', '', $this->get_url_for_language( $default_language ) );
        if ( apply_filters( 'translentor-exclude-hreflang', false, $original_url ) ) {
            return;
        }

        $languages = $this->settings['translentor_publish-lang'];
        if ( isset( $_GET['translentor-editor'] ) && $_GET['translentor-editor'] == 'preview' ) {
            $languages = get_option('translentor_settings_elementor')['translentor_lang'];
        }
        ...
        foreach ( $languages as $language ) {
            if( apply_filters( 'translentor_add_country_hreflang_tags', true ) ){
                // hreflang should have - instead of _ . For example: en-EN, not en_EN like the locale
                $hreflang = str_replace( '_', '-', $language );
                $hreflang = apply_filters( 'translentor_hreflang', $hreflang, $language );
            ...
            if ( apply_filters( 'translentor_add_region_independent_hreflang_tags', true ) ) {
        ...
        foreach ($languages as $language){
            ...
            if ( isset( $this->settings['translentor_advanced_settings']['enable_hreflang_xdefault'] ) && $this->settings['translentor_advanced_settings']['enable_hreflang_xdefault'] != 'disabled' ) {
                $default_lang = $this->settings['translentor_advanced_settings']['enable_hreflang_xdefault'];
    ...
    public function change_lang_attr_in_html_tag( $output ){
        ...
        if ( $lang && !empty($translentor_LANGUAGE) ) {
            $output = str_replace( 'lang="'. $lang .'"', 'lang="'. str_replace('_', '-', $translentor_LANGUAGE ) .'"', $output );
        }
     ...
     * @param string $translentor_link_is_processed
     * @return string
     */

    public function get_url_for_language ( $language = null, $url = null, $translentor_link_is_processed = '#translentorLINKPROCESSED') {
	    $debug = false;
	    // initializations
	    global $translentor_LANGUAGE;
        global  $translentor_current_url_term_slug, $translentor_current_url_taxonomy;//these are globals that we set on the 'request' filter in our SEO addon

	    if ( empty($url) ){
		    $url = $this->cur_page_url();
	    }

	    if(apply_filters('translentor_skip_url_for_language', false, $url)){
		    return (string)$url;
	    }

        $hash = hash( 'md4', (string)$language . (string)$url . (string)$translentor_link_is_processed . (string)$translentor_LANGUAGE );
        // set $new_url to false when debugging and developing new features
        $new_url = translentor_cache_get('get_url_for_language_' . $hash, 'translentor');
        if ( $new_url !== false ){
            return $new_url;
        }

        $translentor_language_copy = $translentor_LANGUAGE;
        if ( empty( $language ) ) {
            $language = $translentor_LANGUAGE;
        }

        $url_obj = translentor_cache_get('url_obj_' . hash('md4', $url), 'translentor');
        if( $url_obj === false ){
            $url_obj = new \translentor\Uri($url);
            wp_cache_set('url_obj_' . hash('md4', $url), $url_obj, 'translentor' );
        }

        $abs_home_url_obj = translentor_cache_get('url_obj_' . hash('md4',  $this->get_abs_home() ), 'translentor');
        if( $abs_home_url_obj === false ){
            $abs_home_url_obj = new \translentor\Uri( $this->get_abs_home() );
            wp_cache_set('url_obj_' . hash('md4', $this->get_abs_home()), $abs_home_url_obj, 'translentor' );
        }

        if( $translentor_LANGUAGE == $this->settings['translentor-default-language'] ){
            $translentor_link_is_processed = '';
        }

        // actual logic of the function

        if ( $this->is_sitemap_path('') ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => 'is file'));
            wp_cache_set('get_url_for_language_' . $hash, $url . $translentor_link_is_processed, 'translentor');
            return $url . $translentor_link_is_processed; //abort for files
        }
        ...
        if ( !$url_obj->isSchemeless() && $url_obj->getScheme() != 'http' && $url_obj->getScheme() != 'https' ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => "is different scheme ".$url_obj->getScheme()));
            wp_cache_set('get_url_for_language_' . $hash, $url . $translentor_link_is_processed, 'translentor');
            return $url . $translentor_link_is_processed; // abort for non-http/https links
        }

        if ( $url_obj->isSchemeless() && !$url_obj->getPath() ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => "is anchor or has get params"));
            wp_cache_set('get_url_for_language_' . $hash, $url, 'translentor');
        ...
        if ( $url_obj->getHost() && $abs_home_url_obj->getHost() && $url_obj->getHost() != $abs_home_url_obj->getHost() ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => "is external url "));
            wp_cache_set('get_url_for_language_' . $hash, $url, 'translentor');
            return $url; // abort for external url's
        }

        if( $this->get_lang_from_url_string($url) === null && $this->settings['translentor-default-language'] === $language && $this->settings['translentor-subdirectory-default-lang'] !== 'yes' ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => "URL already has the correct language added to it and default language has subdir"));
            wp_cache_set('get_url_for_language_' . $hash, $url, 'translentor');
            return $url;
        }

        if( $this->get_lang_from_url_string($url) === $language ){
            translentor_bulk_debug($debug, array('url' => $url, 'abort' => "URL already has the correct language added to it"));
            wp_cache_set('get_url_for_language_' . $hash, $url, 'translentor');
            return $url;
        }

        // maybe find the post_id for the current URL
        $possible_post_id = translentor_cache_get( 'possible_post_id_'. hash('md4', $url ), 'translentor' );
        if ( $possible_post_id ){
            $post_id = $possible_post_id;
            translentor_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for language' => $translentor_LANGUAGE));
        } else {
            $post_id = url_to_postid( $url );
            wp_cache_set( 'possible_post_id_' . hash('md4', $url ), $post_id, 'translentor' );
            if ( $post_id ) { translentor_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for default language' => $translentor_LANGUAGE)); }
            ...
            if ( $post_id == 0 ) {
                ...
                */
                $translentor_LANGUAGE = $this->settings['translentor-default-language'];
                ...
                */
                $possible_url = $url;
				if (isset ($this->settings['translentor-subdirectory-default-lang']) && $this->settings['translentor-subdirectory-default-lang'] === 'yes' && $this->get_lang_from_url_string( $url ) == null ){
                ...
                wp_cache_set( 'possible_post_id_' . hash('md4', $possible_url ), $post_id, 'translentor' );
                if($post_id){ translentor_bulk_debug($debug, array('url' => $url, 'found post id' => $post_id, 'for default language' => $translentor_LANGUAGE)); }
                $translentor_LANGUAGE = $translentor_language_copy;
            }
        }
        ...
        if( $post_id ){
            ...
            translentor_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'found post id' => $post_id, 'url type' => 'based on permalink', 'for language' => $translentor_LANGUAGE));
            $translentor_LANGUAGE = $translentor_language_copy;

        }else if( isset( $translentor_current_url_term_slug ) && isset($translentor_current_url_taxonomy) &&
            !is_wp_error( get_term_link( $translentor_current_url_term_slug, $translentor_current_url_taxonomy)) &&
            strpos( urldecode( $url ), get_term_link( $translentor_current_url_term_slug, $translentor_current_url_taxonomy) ) === 0
        ){ // check here if it is a term link
            $current_term_link = get_term_link( $translentor_current_url_term_slug, $translentor_current_url_taxonomy);
            $translentor_LANGUAGE = $language;
                $check_term_link = get_term_link($translentor_current_url_term_slug, $translentor_current_url_taxonomy);
            ...
            if ($this->get_lang_from_url_string($url) === null) {
                ...
                translentor_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'lang' => $language, 'url type' => 'custom url without language parameter'));
            } else {
                ...
                translentor_bulk_debug($debug, array('url' => $url, 'new url' => $new_url, 'lang' => $language, 'url type' => 'custom url with language', 'abs home path' => $abs_home_url_obj->getPath()));
            }
        }
        ...
        if( class_exists( 'WooCommerce' ) ){
            ...
            foreach ($english_woocommerce_slugs as $english_woocommerce_slug){
                ...
                $current_slug = translentor_get_transient( 'tp_'.$english_woocommerce_slug.'_'. $this->settings['translentor-default-language'] );
                if( $current_slug === false ){
                    $current_slug = translentor_x( $english_woocommerce_slug, 'slug', 'woocommerce', $this->settings['translentor-default-language'] );
                    set_transient( 'tp_'.$english_woocommerce_slug.'_'. $this->settings['translentor-default-language'], $current_slug, 12 * HOUR_IN_SECONDS );
                }
                ...
                if( $this->translentor_get_woocommerce_saved_permalink($english_woocommerce_slug) === $current_slug ) {

                    if (strpos($new_url, '/' . $current_slug . '/') === false) {
                        $current_slug = translentor_get_transient('tp_' . $english_woocommerce_slug . '_' . $translentor_LANGUAGE);
                        if ($current_slug === false) {
                            $current_slug = translentor_x($english_woocommerce_slug, 'slug', 'woocommerce', $translentor_LANGUAGE);
                            set_transient('tp_' . $english_woocommerce_slug . '_' . $translentor_LANGUAGE, $current_slug, 12 * HOUR_IN_SECONDS);
                        }
                    }

                    $translated_slug = translentor_get_transient('tp_' . $english_woocommerce_slug . '_' . $language);
                    if ($translated_slug === false) {
                        $translated_slug = translentor_x($english_woocommerce_slug, 'slug', 'woocommerce', $language);
	    ...
	    $new_url = apply_filters( 'translentor_get_url_for_language', $new_url, $url, $language, $this->get_abs_home(), $this->get_lang_from_url_string($url), $this->get_url_slug( $language ) );
        wp_cache_set('get_url_for_language_' . $hash, $new_url . $translentor_link_is_processed, 'translentor');
        return $new_url . $translentor_link_is_processed ;
    ...
    public function url_is_file( $url = null ){
        ...
        if ( empty( $url ) || $translation_render->is_external_link($url) ){
            ...
            if ( strpos( $url, 'wp-content/uploads' ) !== false ) {
                ...
                if(apply_filters('translentor_is_file', true, $path)) {
                    $return = is_file( $path );
                }else{
                    $return = true;
                }
        ...
        return apply_filters( 'translentor_url_is_file', $return, $url, $this->get_abs_home() );
    }

    public function does_url_contains_array($return, $path){
        $elements_to_avoid = apply_filters( 'translentor_elements_to_avoid_when_is_file_is_called', array("translentor.php", "/../"));
     ...
     * @param bool $accept_empty_return     Whether to take into account the translentor-subdirectory-default-lang setting.
     * @return string                       Url slug.
     */
    public function get_url_slug( $language_code, $accept_empty_return = true ){
        $url_slug = $language_code;
        if( isset( $this->settings['translentor-lang-slugs'][$language_code] ) ) {
            $url_slug = $this->settings['translentor-lang-slugs'][$language_code];
        }

        if ( $accept_empty_return && isset( $this->settings['translentor-subdirectory-default-lang'] ) && $this->settings['translentor-subdirectory-default-lang'] == 'no' && $language_code == $this->settings['translentor-default-language'] ) {
            $url_slug = '';
        }
    ...
    public function get_abs_home() {
	    $this->absolute_home = translentor_cache_get('get_abs_home', 'translentor');
        ...
        if ( apply_filters('translentor_adjust_absolute_home_https_based_on_server_variable', true) ) {
	    ...
	    wp_cache_set( 'get_abs_home', $this->absolute_home, 'translentor' );
    ...
    public function get_lang_from_url_string( $url = null ) {
        ...
        $language = translentor_cache_get('url_language_' . hash('md4', $url) , 'translentor' );
        if ( $language !== false ){
            return $language;
        }
        ...
        if( $url_obj->getPath() ){
            ...
            $lang = explode('/', $lang);
            if( $lang == false ){
                wp_cache_set('url_language_' . hash('md4', $url), null, 'translentor');
                return null;
            }
            // If we have a language in the URL, the first element of the array should be it.
            $lang = $lang[0];

            $lang = apply_filters( 'translentor_get_lang_from_url_string', $lang, $url );

            // the lang slug != actual lang. So we need to do array_search so we don't end up with en instead of en_US
            if( isset($this->settings['translentor-lang-slugs']) && in_array($lang, $this->settings['translentor-lang-slugs']) ){
                $language = array_search($lang, $this->settings['translentor-lang-slugs']);
                if ( in_array( $language, $this->settings['translentor_publish-lang'] ) ||
                    ( in_array( $language, get_option('translentor_settings_elementor')['translentor_lang'] ) && current_user_can(apply_filters( 'translentor_translating_capability', 'manage_options' )) ) ) {
                    wp_cache_set( 'url_language_' . hash( 'md4', $url ), $language, 'translentor' );
                    return $language;
                }
            }
        }
    ...
    public function cur_page_url() {
        ...
        $req_uri = translentor_cache_get('cur_page_url', 'translentor');
        if ( $req_uri ){
            return $req_uri;
        }
        ...
        if ( function_exists('apply_filters') ) $req_uri = apply_filters('translentor_curpageurl', $req_uri);
        wp_cache_set('cur_page_url', $req_uri, 'translentor');
        return $req_uri;
    }
        ...
        if( class_exists( 'WooCommerce' ) ){
            ...
            if( $translentor_LANGUAGE != $this->settings['translentor-default-language'] ){
                ...
                if( empty($default_language_wc_permalink_structure) ) {
                    $default_language_wc_permalink_structure = translentor_get_transient( 'tp_default_language_wc_permalink_structure_'.$this->settings['translentor-default-language'] );
                    if( $default_language_wc_permalink_structure === false ) {
                        $default_language_wc_permalink_structure = array();
                        $default_language_wc_permalink_structure['product_rewrite_slug'] = translentor_x('product', 'slug', 'woocommerce', $this->settings['translentor-default-language']);
                        $default_language_wc_permalink_structure['category_rewrite_slug'] = translentor_x('product-category', 'slug', 'woocommerce', $this->settings['translentor-default-language']);
                        $default_language_wc_permalink_structure['tag_rewrite_slug'] = translentor_x('product-tag', 'slug', 'woocommerce', $this->settings['translentor-default-language']);

                        set_transient('tp_default_language_wc_permalink_structure_' . $this->settings['translentor-default-language'], $default_language_wc_permalink_structure, 12 * HOUR_IN_SECONDS);
                    }
                }

                $current_language_permalink_structure = translentor_get_transient( 'tp_current_language_wc_permalink_structure_'.$translentor_LANGUAGE );
                if( $current_language_permalink_structure === false ) {
                    ...
                    $current_language_permalink_structure['product_rewrite_slug'] = translentor_x('product', 'slug', 'woocommerce', $translentor_LANGUAGE);
                    $current_language_permalink_structure['category_rewrite_slug'] = translentor_x('product-category', 'slug', 'woocommerce', $translentor_LANGUAGE);
                    $current_language_permalink_structure['tag_rewrite_slug'] = translentor_x('product-tag', 'slug', 'woocommerce', $translentor_LANGUAGE);

                    set_transient( 'tp_current_language_wc_permalink_structure_'.$translentor_LANGUAGE, $current_language_permalink_structure, 12 * HOUR_IN_SECONDS );
                }
    ...
    function woocommerce_filter_permalink_option( $value ){
        global $translentor_LANGUAGE, $translentor_wc_permalinks;

        //keep the unfiltered value in a global, we might need it later
        if( !isset( $translentor_wc_permalinks ) )
            $translentor_wc_permalinks = $value;

        if( $translentor_LANGUAGE != $this->settings['translentor-default-language'] ) {
            if( trim($value['product_base'], '/') === translentor_x( 'product', 'slug', 'woocommerce', $this->settings['translentor-default-language'] ) ){
                $value['product_base'] = '';
                ...
                if( wp_doing_ajax() ){
                    $value['product_base'] = translentor_x( 'product', 'slug', 'woocommerce', $translentor_LANGUAGE );
                }
            }else{
	            ...
	            if ( substr( $value['product_base'], 0, strlen('/product/' ) ) === '/product/' ) {
		            $value['product_base'] = substr_replace( $value['product_base'], '/' . translentor_x( 'product', 'slug', 'woocommerce', $translentor_LANGUAGE ) . '/', 0, strlen('/product/' ) );
	            }
            }

            if( trim($value['category_base'], '/') === translentor_x( 'product-category', 'slug', 'woocommerce', $this->settings['translentor-default-language'] ) ){
                $value['category_base'] = '';
            }

            if( trim($value['tag_base'], '/') === translentor_x( 'product-tag', 'slug', 'woocommerce', $this->settings['translentor-default-language'] ) ){
                $value['tag_base'] = '';
            }
    ...
    function prevent_permalink_update_on_other_languages( $value, $old_value ){
        global $translentor_LANGUAGE;
        if( isset($translentor_LANGUAGE) && $translentor_LANGUAGE != $this->settings['translentor-default-language'] && apply_filters( 'translentor_prevent_permalink_update_on_other_languages', true ) ) {
            $value = $old_value;
        }
     ...
     */
    function translentor_get_woocommerce_saved_permalink( $english_woocommerce_slug ){
