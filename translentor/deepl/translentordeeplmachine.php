...
class translentor_IN_Deepl_Machine_Translator extends translentor_Machine_Translator {
    /**
    ...
    public function translate_array( $new_strings, $target_language_code, $source_language_code = null ){
        ...
        if ( $source_language_code == null )
            $source_language_code = get_option('translentor_settings_elementor')['translentor-default-language'];
        ...
        $source_language = apply_filters( 'translentor_deepl_source_language', $this->machine_translation_codes[$source_language_code], $source_language_code, $target_language_code );
        $target_language = apply_filters( 'translentor_deepl_target_language', $this->machine_translation_codes[$target_language_code], $source_language_code, $target_language_code );
    ...
    public function get_languages_that_support_formality(){
        ...
        //if (isset($data['translentor_mt_supported_languages'][$this->settings['translentor_machine_translation_settings']['translation-engine']]['formality-supported-languages'])){
            foreach (get_option('translentor_settings_elementor')['translentor-lang-slugs'] as $language){
                if(array_key_exists($language, $data['translentor_mt_supported_languages']['deepl']['formality-supported-languages'])){
                    $formality_supported_languages[$language] = $data['translentor_mt_supported_languages'][$this->settings['translentor_machine_translation_settings']['translation-engine']]['formality-supported-languages'][$language];
                }else{
                    $this->check_languages_availability(get_option('translentor_settings_elementor')['translentor-lang-slugs'], true);
                    $data = get_option('translentor_db_stored_data', array());
                    $formality_supported_languages = $data['translentor_mt_supported_languages'][$this->settings['translentor_machine_translation_settings']['translation-engine']]['formality-supported-languages'];
                    break;
                }
            }
    ...
    public function check_formality(){
        ...
        foreach (get_option('translentor_settings_elementor')['translentor-lang-slugs'] as $value=>$language) {

            $target_language = apply_filters( 'translentor_deepl_target_language', $this->machine_translation_codes[$language], null, $language );
    ...
    public function get_engine_specific_language_codes($languages){
        ...
        $iso_translation_codes = get_option('translentor_settings_elementor')['translentor-lang-slugs'];
        ...
        foreach( $languages as $language ) {
            ...
            */
            ...
            $engine_specific_languages[] = apply_filters( 'translentor_deepl_source_language', $iso_translation_codes[ $language ], $language, null );
        }
        return $engine_specific_languages;
    }

    public function get_api_url(){
       if( get_option('fp_deepl')=='free' )
       {
