...
class translentor_String_Translation_Helper {
    /* @var Lang_Query */
    protected $lang_query;
    /* @var translentor_String_Translation */
    ...
    public function check_ajax( $type, $get_or_save ) {
        if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
            ...
            $action = ( $get_or_save === 'save' ) ? 'translentor_save_translations_' : 'translentor_string_translation_get_strings_';
    ...
    public function get_sanitized_query_args( $string_type ) {
        ...
        if ( !$this->string_translation ) {
            $this->string_translation = $translentor->get_translentorelements( 'string_translation' );
        }
        if ( !$this->lang_query ) {
        ...
        foreach ( $translation_status_filters['translation_status'] as $translation_status_key => $value ) {
            if ( !empty( $posted_query[ $translation_status_key ] ) && ( $posted_query[ $translation_status_key ] === true || $posted_query[ $translation_status_key ] === 'true' ) ) {
                ...
                $query_args['status'][] = $this->lang_query->$constant_func_name();
            }
        }
        ...
        // language
        $query_args['language'] = ( !empty( $posted_query['language'] ) && in_array( $posted_query['language'], get_option('translentor_settings_elementor')['translentor_lang'] ) ) ?
        ...
        return apply_filters( 'translentor_sanitized_query_args', $query_args, $string_type, $string_types );
    }
