...
class translentor_IN_ALD_Ajax{
    ...
    /**
     * translentor_IN_ALD_Ajax constructor. Echos the preferred language.
     *
     */
    public function __construct(){
		
        if ( !isset( $_POST['action'] ) || $_POST['action'] !== 'translentor_translentor_get_needed_language' || empty( $_POST['detection_method'] ) || empty( $_POST['publish_languages'] ) || empty( $_POST['iso_codes'] ) ) {
            die();
        }
	
        require_once( __DIR__ . '/class-determine-language.php' );
	    $translentor_determine_language = new translentor_IN_ALD_Determine_Language();
	    ...
	    $needed_language = $translentor_determine_language->get_needed_language( $published_languages, $iso_codes, $detection_method );
