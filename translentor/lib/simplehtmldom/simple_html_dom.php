...
/*
 * translentor modifications
 *
 * "translentor_" prefix was appended to all the define constants.
 *
 * namespace translentor was added.
 *
 * MAX_FILE_SIZE has been modified from value 600000 to new value: define( 'translentor_MAX_FILE_SIZE', 100000000 );
 *
 * In class simple_html_dom_node, property $tag has been renamed from 'text' to 'translentortext'.
 * Also, all other 3 occurrences of 'text' where changed to 'translentortext'.
 * (Reason for this change: tag <text> exists in HTML docs so we need a different reserved tag.)
 * (In class-translation-render.php we use find('translentortext') because of this. ).
 *
...
define('translentor_HDOM_TYPE_ELEMENT', 1);
define('translentor_HDOM_TYPE_COMMENT', 2);
define('translentor_HDOM_TYPE_TEXT', 3);
define('translentor_HDOM_TYPE_ENDTAG', 4);
define('translentor_HDOM_TYPE_ROOT', 5);
define('translentor_HDOM_TYPE_UNKNOWN', 6);
define('translentor_HDOM_QUOTE_DOUBLE', 0);
define('translentor_HDOM_QUOTE_SINGLE', 1);
define('translentor_HDOM_QUOTE_NO', 3);
define('translentor_HDOM_INFO_BEGIN', 0);
define('translentor_HDOM_INFO_END', 1);
define('translentor_HDOM_INFO_QUOTE', 2);
define('translentor_HDOM_INFO_SPACE', 3);
define('translentor_HDOM_INFO_TEXT', 4);
define('translentor_HDOM_INFO_INNER', 5);
define('translentor_HDOM_INFO_OUTER', 6);
define('translentor_HDOM_INFO_ENDSPACE', 7);

defined('translentor_DEFAULT_TARGET_CHARSET') || define('translentor_DEFAULT_TARGET_CHARSET', 'UTF-8');
defined('translentor_DEFAULT_BR_TEXT') || define('translentor_DEFAULT_BR_TEXT', "\r\n");
defined('translentor_DEFAULT_SPAN_TEXT') || define('translentor_DEFAULT_SPAN_TEXT', ' ');
defined('translentor_MAX_FILE_SIZE') || define('translentor_MAX_FILE_SIZE', 100000000);
define('translentor_HDOM_SMARTY_AS_TEXT', 1);
...
function file_get_html(
	...
	$target_charset = translentor_DEFAULT_TARGET_CHARSET,
	$stripRN = true,
	$defaultBRText = translentor_DEFAULT_BR_TEXT,
	$defaultSpanText = translentor_DEFAULT_SPAN_TEXT)
{
	if($maxLen <= 0) { $maxLen = translentor_MAX_FILE_SIZE; }
	...
	$defaultSpanText = translentor_DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen($str) > translentor_MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
...
{
	public $nodetype = translentor_HDOM_TYPE_TEXT;
	public $tag = 'translentortext';
	public $attr = array();
	...
	function dump_node($echo = true)
	{
		...
		if (isset($node->_[translentor_HDOM_INFO_INNER])) {
			$string .= "'" . $node->_[translentor_HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[translentor_HDOM_INFO_INNER])) {
			return $this->_[translentor_HDOM_INFO_INNER];
		}

		if (isset($this->_[translentor_HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[translentor_HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[translentor_HDOM_INFO_OUTER])) {
			return $this->_[translentor_HDOM_INFO_OUTER];
		}
		...
		if ($this->dom && $this->dom->nodes[$this->_[translentor_HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[translentor_HDOM_INFO_BEGIN]]->makeup();
		}

		if (isset($this->_[translentor_HDOM_INFO_INNER])) {
			// todo: <br> should either never have translentor_HDOM_INFO_INNER or always
			if ($this->tag !== 'br') {
				$ret .= $this->_[translentor_HDOM_INFO_INNER];
			}
		} elseif ($this->nodes) {
			foreach ($this->nodes as $n) {
		...
		if (isset($this->_[translentor_HDOM_INFO_END]) && $this->_[translentor_HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case translentor_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[translentor_HDOM_INFO_TEXT]);
			case translentor_HDOM_TYPE_COMMENT: return '';
			case translentor_HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or translentor_HDOM_TYPE_ELEMENT - observed
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[translentor_HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[translentor_HDOM_INFO_QUOTE][$i])
				{
					case translentor_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case translentor_HDOM_QUOTE_SINGLE: $quote = '\''; break;
                    /*
                     * translentor modifications
                     * We changed the default $quote from '' to ' " ' to avoid JS security issue. There is a filter applied on this global, $translentor_HDOM_QUOTE_DEFAULT,
                     * so it can be changed in case there is a problem for some clients.
*/
					default: { global $translentor_HDOM_QUOTE_DEFAULT; $quote = $translentor_HDOM_QUOTE_DEFAULT; };
  				}

				$ret .= $key
				. $this->_[translentor_HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[translentor_HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[translentor_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[translentor_HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[translentor_HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[translentor_HDOM_INFO_END])) ? $this->_[translentor_HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[translentor_HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[translentor_HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[translentor_HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			// Handle 'translentortext' selector
			if($pass && $tag === 'translentortext' && $node->tag === 'translentortext') {
			...
			if ($pass) $ret[$node->_[translentor_HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[translentor_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[translentor_HDOM_INFO_TEXT])) {
					return $this->_[translentor_HDOM_INFO_TEXT] = $value;
				}
				return $this->_[translentor_HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[translentor_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[translentor_HDOM_INFO_QUOTE][] = translentor_HDOM_QUOTE_DOUBLE;
		}
...
class simple_html_dom
{
		...
		$options = 0)
	{
		...
		if($options & translentor_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[translentor_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = translentor_DEFAULT_SPAN_TEXT)
	{
		...
		$this->root->tag = 'root';
		$this->root->_[translentor_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = translentor_HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			++$this->cursor;
			$node->_[translentor_HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[translentor_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[translentor_HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[translentor_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new simple_html_dom_node($this);
		$node->_[translentor_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[translentor_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = translentor_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = translentor_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[translentor_HDOM_INFO_TEXT] .= '>'; }
		...
		if ($pos = strpos($tag, '<') !== false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[translentor_HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[translentor_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = translentor_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = translentor_HDOM_TYPE_TEXT;
				$node->_[translentor_HDOM_INFO_END] = 0;
				$node->_[translentor_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'translentortext';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[translentor_HDOM_INFO_END] = 0;
				$node->_[translentor_HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[translentor_HDOM_INFO_QUOTE][] = translentor_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[translentor_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[translentor_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[translentor_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[translentor_HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[translentor_HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"':
				$quote_type = translentor_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('"');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'':
				$quote_type = translentor_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('\'');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			default:
				$quote_type = translentor_HDOM_QUOTE_NO;
		...
		if (!$is_duplicate) {
			$node->_[translentor_HDOM_INFO_QUOTE][] = $quote_type;
